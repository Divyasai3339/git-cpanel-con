<?php
    ...
    class MedicalCamp extends Admin_Controller
    {
        ...
        public function __construct()
        {
            ...
            $this->load->model(array('conference_model', 'medicalcamp_model','transaction_model', 'casereference_model', 'patient_model', 'notificationsetting_model'));
        ...
        {
            if (!$this->rbac->hasPrivilege('medical_camp', 'can_view')) {
                access_denied();
            }
            ...
            $this->session->set_userdata('top_menu', 'medical_camp');
            ...
            $this->load->view('admin/medicalcamp/index', $data);
     ...
     */
        ...
        public function addpatient()
        {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $insert_id = $this->patient_model->add_patientcamp($patient_data);
        ...
        public function getmedicalcampdatatable()
        {
            $dt_response = $this->patient_model->getAllcampRecord();
            ...
            if (!empty($dt_response->data)) {
                foreach ($dt_response->data as $key => $value) {
                   $row = array();
                    //====================================
                    $getid = $this->db->query("select * from medicalcamp where patient_id = $value->id");
                  
                    if(!empty($getid->row())){
                    $get_result = $getid->row();
                    $action = "<div>";
                    
                        $action .= "<a href='#'  class='btn btn-default btn-xs viewOptometryData' data-opto_id ='" . $get_result->id. "' data-opd='" . $get_result->id . "' data-toggle='tooltip' title='VIEW EMR'><i class='fas fa fa-eye'></i></a>";
                  
                        
                    }
                    else {
                        $action = "<a href='#' onclick='getRecord_emr(" . $value->id . "," .$value->id. ")' class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('emr') . "'><i class='fas fa fa-plus'></i></a>";
                    }
        ...
        public function getopdvisitdatatable()
        {
            ...
            if (!empty($dt_response->data)) {
                foreach ($dt_response->data as $key => $value) {
                    ...
                    //} else {
                    $action .= "<a href='#'  class='btn btn-default btn-xs btnUploadAudiometryImagesG' data-opd=" . $opd_id . "   data-toggle='tooltip' title='Upload Audiometry Photos'><i class='fa fa-upload'></i></a>";
                    // }
                    if ($optom) {
                        $action .= "<a href='#'  class='btn btn-default btn-xs viewOptometryData' data-opto_id ='" . $optom->id . "' data-opd='" . $opd_id . "' data-toggle='tooltip' title='VIEW EMR'><i class='fas fa fa-eye'></i></a>";
                    } else {
                        $action .= "<a href='#' onclick='getRecord_emr(". $visit_details_id. "," . $patientid.  ")' class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('emr') . "'><i class='fas fa fa-plus'></i></a>";
                    }
        ...
        public function ipdprofile($ipdid)
        {
            ...
            $data['investigations']      = $this->patient_model->getallinvestigation($result['case_reference_id']);
             $data['consumables']         = $this->patient_model->getallconsumable($ipdid);
        ...
        // public function getpatientDetails()
        // {
        //     // echo "i am here finally";die;
        //     $id                    = $this->input->post("id");getmedicalcamppatientDetails
        //     // $result                = $this->patient_model->getpatientDetails($id);
        //     $result['patient_age'] = $this->customlib->getPatientAge($result['age'], $result['month'], $result['day']);
        ...
        {
            $id                    = $this->input->post("id");
            $result                = $this->patient_model->getmedicalcamppatientDetails($id);
            $result['patient_age'] = $this->customlib->getPatientAge($result['age'], $result['month'], $result['day']);
        ...
        //         $footer_note         = $this->input->post("footer_note");
        //         $ipd_no_value        = $this->input->post('ipd_no_value');
        //         // $finding_type        = $this->input->post('finding_type');
        //         //$complaints_type     = $this->input->post('complaint_type_id');
        //         $finding_description = $this->input->post('finding_description');
        //         $finding_print       = $this->input->post('finding_print');
    
        //         $complaints_description = $this->input->post('complaints_description');
        //         $complaints_print       = $this->input->post('complaints_print');
        ...
        //         }
        //         $examinations = $this->input->post('examinations');
        //         $diagnosis = $this->input->post('diagnosis');
        //         $opd_procedure = $this->input->post('opd_procedure');
        ...
        //             'header_note'         => $header_note,
        //             'footer_note'         => $footer_note,
        //             'finding_categoryid'  => $this->input->post('finding_type_id'),
        //             'complaint_categoryid'=> $this->input->post('complaint_type_id'),
        //             'finding_description' => $finding_description,
        //             'complaints_description' => $complaints_description,             
        //             'is_finding_print'    => $finding_print,
        //             'is_complaints_print' => $complaints_print,
        //             'complaints'          => json_encode($complaints),
        //             'history'             => json_encode($history),
        //             'examinations'        => json_encode($examinations),
        //             'diagnosis'           => json_encode($diagnosis),
        //             'opd_procedure'        => json_encode($opd_procedure),
        //             'surgery_recommendations' => json_encode($surgery_recommendations),
        //             'date'                => date("Y-m-d"),
        //             'generated_by'        => $this->customlib->getStaffID(),
        ...
        //             // 'finding_type'        => $finding_category_id,
        //              'complaints_type'     => $complaint_category_id,
        //             'finding_description' => $finding_description,
        //             'complaints_description' => $complaints_description,               
        ...
        public function add_opd_prescription()
        {
            ...
            if (isset($total_rows) && !empty($total_rows)) {
                foreach ($total_rows as $row_key => $row_value) {
                    ...
                    if ($medicine_category !== "") {
                        ...
                        if ($medicine_name == "") {
                            $this->form_validation->set_rules('medicine_name', $this->lang->line('medicine'), 'trim|xss_clean');
                        }
            ...
            if ($this->form_validation->run() == false) {
                ...
                $total_complain               = $this->input->post('complainrow');
                $complaints_description       = array();
                if(!empty($total_complain)){
                    foreach ($total_complain as $row_key => $row_value) {
                        $complaints_description[] = array(
                            'complaint_type_id'      => $this->input->post("complaint_type_id_" . $row_value),
                            'complaints_description' => json_encode($this->input->post("complaints_description_" . $row_value)),
                            'durations'              => $this->input->post("durations_" . $row_value),
                            'periods'                => $this->input->post("periods_" . $row_value),
                        );
                    }
                }
    
                $total_finding               = $this->input->post('findingrow');
                $finding_description       = array();
                if(!empty($total_complain)){
                    foreach ($total_finding as $row_key => $row_value) {
                        $finding_description[] = array(
                            'finding_type_id'      => $this->input->post("finding_type_id_" . $row_value),
                            'finding_description' => json_encode($this->input->post("finding_description_" . $row_value)),
                        );
                    }
                }
                ...
                // $finding_type        = $this->input->post('finding_type');
                //$complaints_type     = $this->input->post('complaint_type_id');
                $finding_description = json_encode($finding_description);//$this->input->post('finding_description');
                //$finding_print       = $this->input->post('finding_print');
    
                $complaints_description = json_encode($complaints_description);//$this->input->post('complaints_description');
                //$complaints_print       = $this->input->post('complaints_print');
                ...
                foreach ($post_datas['durations'] as $key => $val) {
                    $complaints[$key]['durations'] = $val;
                }
                foreach ($post_datas['periods'] as $key => $val) {
                    $complaints[$key]['periods'] = $val;
                }
                $history = array();
                $post_data = $this->input->post();
                foreach ($post_data['disease'] as $key => $val) {
                ...
                $opd_basic_array = array(
                    'visit_details_id'    => $visitid,
                    'header_note'         => $header_note,
                    'finding_categoryid'  => 0,//$this->input->post('finding_type_id'),
                    'complaint_categoryid'=> 0,//$this->input->post('complaint_type_id'),
                    'finding_description' => $finding_description,
                    'complaints_description' => $complaints_description,             
                    'is_finding_print'    => 'yes',//$finding_print,
                    'is_complaints_print' => 'yes',//$complaints_print,
                    'complaints'          => json_encode($complaints),
                    'history'             => json_encode($history),
                    'examinations'        => json_encode($examinations),
                    'diagnosis'           => json_encode($diagnosis),
                    'opd_procedure'        => json_encode($opd_procedure),
                    'surgery_recommendations' => json_encode($surgery_recommendations),
                    'footer_note'          => json_encode($footer_note),
                    'date'                => date("Y-m-d"),
                    'generated_by'        => $this->customlib->getStaffID(),
                    'prescribe_by'        => $opd_details['doctor_id'],
                );
        ...
        public function add_template_prescription()
        {
           
            $total_rows          = $this->input->post('rows');
            $this->form_validation->set_rules('medicine_type', $this->lang->line("medicine_type"), 'trim|required|xss_clean');
            $medicine = 0;
            if (isset($total_rows) && !empty($total_rows)) {
                foreach ($total_rows as $row_key => $row_value) {
                    $medicine_category = $this->input->post('medicine_cat_' . $row_value);
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($medicine == 1) {
                    foreach ($total_rows as $row_key => $row_value) {
                            ...
                            $insert_medicines[] = array(
                                'template_id'    => $this->input->post("medicine_type"),
                                'med_category_id'=> $this->input->post("medicine_cat_" . $row_value),
                                'medicine_id'      => $this->input->post("medicine_" . $row_value),
                                'dosage'           => $this->input->post("dosage_" . $row_value),
                                'dose_interval_id' => $this->input->post("interval_dosage_" . $row_value),
                                'dose_duration_id' => $this->input->post("duration_dosage_" . $row_value),
                                'instruction'      => $this->input->post("instruction_" . $row_value),
                ...
                $insert_id       = $this->prescription_model->add_template_prescription($insert_medicines);
        ...
        public function medicalcamp_report()
        {
            ...
            $this->session->set_userdata('sub_menu', 'admin/medicalcamp/medicalcamp_report');
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/medicalcamp/mediaclcampReport', $data);
            $this->load->view('layout/footer');
        }
        ...
        public function opdreports()
        {
            ...
            $reportdata = $this->transaction_model->mcamppatientreportRecord($data);
            $reportdata = json_decode($reportdata);
            ...
            if (!empty($reportdata->data)) {
                foreach ($reportdata->data as $key => $value) {
                    ...
                    $first_action = "<a href=" . base_url() . 'admin/patient/profile/' . $value->id . ">";
        ...
        {
            $id                   = $_REQUEST['complaints_id'];
            $data['row_id_val'] = $_REQUEST['row_id_val'];
            $data['complaints_list'] = $this->complaints_model->getbyfinding($id);
            $section_page         = $this->load->view('admin/patient/_getcomplaintslist.php', $data, true);
        ...
        public function get_template()
        {
            ...
            $query = $this->db->get('med_template_details');
            $template_data = $query->result_array();
            $medicineCategory   = $this->medicine_category_model->getMedicineCategory();
            $intervaldosage  = $this->medicine_dosage_model->getIntervalDosage();
            $durationdosage  = $this->medicine_dosage_model->getDurationDosage();
            $category_dosage            = $this->medicine_dosage_model->getMedicineDosage();
            $html="";
           // $html='<tr id="row0"><div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line('medicine_category').'</label></div><div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line('medicine').'</label></div>';
           // $html.='<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line("dose").'</label></div><div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line("dose_interval").'</label></div>';
            //$html.='<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line("dose_duration").'</label></div><div class="col-lg-2 col-md-4 col-sm-6 col-xs-6"><label>'.$this->lang->line('instruction').'</label></div></tr>';
            $i=1;
            foreach($template_data as $val):
                $html.='<tr id="row'.$i.'"><td><input type="hidden" name="rows[]" value="'.$i.'"><div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
                $html.= '<label>'.$this->lang->line('medicine_category').'</label>';
                $html.='<select class="form-control select2 medicine_category" style="width: 100%" name="medicine_cat_'.$i.'">';
                $html.='<option value="'.set_value('medicine_category_id').'">'.$this->lang->line('select').'</option>';
                foreach ($medicineCategory as $dkey => $dvalue) {
                   $catsel = $val['med_category_id']==$dvalue["id"]?'selected':'';
                    $html.='<option value="'.$dvalue["id"].'"'.$catsel.'>'.$dvalue["medicine_category"].'</option>';
                }
                $html.='</select></div></div>';
                $med_data = $this->pharmacy_model->get_medicine_name($val['med_category_id']);
                $html.='<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
                $html.='<label>'.$this->lang->line('medicine').'</label>';
                 $html.= '<select class="form-control select2 medicine_name" data-rowid="'.$i.'" style="width: 100%"  name="medicine_'.$i.'">
                        <option value="">'.$this->lang->line('select').'</option>';
                        foreach ($med_data as $dkey => $dvalue) {
                            $msel = $val['medicine_id']==$dvalue["id"]?'selected':'';
                             $html.='<option value="'.$dvalue["id"].'"'.$msel.'>'.$dvalue["medicine_name"].'</option>';
                         }
                $html.= '</select><div id="suggesstion-box0"><small id="stock_info_1"> </small></div></div>';
    
                $html.='<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
                $html.= '<label>'.$this->lang->line("dose").'</label>';
                $html.='<select class="form-control select2 medicine_dosage" style="width: 100%"  name="dosage_'.$i.'">
                        <option value="">'.$this->lang->line('select').'</option>';
                        foreach ($category_dosage as $dkey => $dvalue) {
                            $mdsel = $val['dosage']==$dvalue["id"]?'selected':'';
                             $html.='<option value="'.$dvalue["id"].'"'.$mdsel.'>'.$dvalue["dosage"].$dvalue["unit"].'</option>';
                         }
                $html.= '</select></div>';
                $html.= '<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
                $html.= '<label>'.$this->lang->line("dose_interval").'</label>'; 
                $html.= '<select class="form-control  select2 interval_dosage" style="width:100%" id="interval_dosage_id" name="interval_dosage_'.$i.'">
                        <option value="'.set_value('interval_dosage_id').'">'.$this->lang->line('select').'</option>';
                         foreach ($intervaldosage as $dkey => $dvalue) {
                           $idsel = $val['dose_interval_id']==$dvalue["id"]?'selected':'';
                           $html.= '<option value="'.$dvalue["id"].'"'.$idsel.'>'.$dvalue["name"].'</option>';
                        }
               $html.= '</select><span class="text-danger">'.form_error('interval_dosage_id').'</span></div>';         
               $html.= '<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
               $html.= '<label>'.$this->lang->line("dose_duration").'</label>'; 
               $html.= '<select class="form-control  select2" style="width:100%" id="interval_dosage_id" name="duration_dosage_'.$i.'">
                       <option value="'.set_value('interval_dosage').'">'.$this->lang->line('select').'</option>';
                        foreach ($durationdosage as $dkey => $dvalue) {
                          $dosesel = $val['dose_duration_id']==$dvalue["id"]?'selected':'';
                          $html.= '<option value="'.$dvalue["id"].'"'.$dosesel.'>'.$dvalue["name"].'</option>';
                           }
                $html.= '</select><span class="text-danger">'.form_error('interval_dosage_id').'</span></div>';
                $html.= '<div class="col-lg-2 col-md-4 col-sm-6 col-xs-6">';
                $html.= '<label>'.$this->lang->line('instruction').'</label>'; 
                 $html.= '<textarea name="instruction_'.$i.'" style="height: 28px;" class="form-control" >'.$val['instruction'].'</textarea>';
                $html.= '</div></div></td>';
                $html.='<td><button type="button" class="closebtn delete_row crossbtnfa"  data-row-id="'.$i.'" autocomplete="off"><i class="fa fa-remove"></i></button></td></tr>';
            $i++;
            endforeach;    
            echo $html;
        }
        ...
        public function allcomplaintsbycategory()
        {
            $id                   = $_REQUEST['complaints_id'];
            $complaints_list = $this->complaints_model->getbyfinding($id);
            echo json_encode($complaints_list);
        }
    
        public function allfindingbycategory()
        {
            $id                   = $_REQUEST['finding_id'];
            $finding_list = $this->finding_model->getbyfinding($id);
            echo json_encode($finding_list);
        }
    
        public function get_medicalcamp_patient(){
            $patient_id    = $this->input->post('patient_id');
            $data          =  $this->patient_model->getMedicalCamppatientbyid($patient_id);
            echo json_encode(array('status' => 1, 'data' => $data));
        }


        public function addMedicalCampData()
        {
            ...
            foreach ($post_data['complaints'] as $key => $val) {
                $complaintss[$key]['complaints'] = $val;
            }
            ...
            // $complaints['od'] = $this->input->post('odcomplaints');
            // $complaints['os'] = $this->input->post('oscomplaints');
            // $complaints['general'] = $this->input->post('generalcomplaints');
            // $complaints['selected_complaints'] = $this->input->post('selected_complaints');
            if (!empty($this->input->post('optometry_id'))) {
                $data = array(
    
                    // 'complaints_data' => json_encode($complaintss),
                    ...
                    'orbit' => $this->input->post('orbit'),
                      'complaints_data' => $this->input->post('complaints_data'),
    
                );
                $res =  $this->master_model->update_data($data, array('id' => $this->input->post('optometry_id')), 'medicalcamp');
                $status = "Updated";
            } else {
                $data = array(
                    'opd_id' => $this->input->post('opd_id'),
                ...
                // echo "<pre>";
            //    print_r($data);die;
                $res =  $this->master_model->insert_data($data, 'medicalcamp');
                $status = "Added";
            }
            if ($res) {
        ...
        public function getMedCampData($id)
        {
            $data = $this->master_model->get_single(array('id' => $id), 'medicalcamp');
            ...
            $patient_data = $this->master_model->get_single_patient(array('id' => $data->patient_id), 'medicalcamp_details');
            ...
            $data = (object) array_merge( (array) $data , (array) $patient_data);
        ...
        public function printemr()
    {
        ...
        $optomery_data 		  = $this->master_model->get_single(array('opd_id' => $id), 'medicalcamp');
        ...
        $opddata                  = $this->master_model->get_single(array('id' => $id), 'medicalcamp_details');
...
// 		echo "here";die;
        ...
        $page = $this->load->view('admin/medicalcamp/_printemr', $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));
     ...
     public function savePrintSettings()
    {
        ...
        $res = $this->master_model->get_single(array('id' => $optoid), 'medicalcamp');
        ...
        $response = $this->master_model->update_data($data, array('id' => $optoid), 'medicalcamp');
        if ($response) {
