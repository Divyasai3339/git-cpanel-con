    ...
    public function addOptimetryData()
    {
        ...
        if (!empty($this->input->post('optometry_id'))) {
            $data = array(
                ...
                'diagnosis_data' => json_encode($diangnosis),
    ...
    public function getOptometryPatientsDataTable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .= "<a href=" . base_url() . 'admin/optometry/visits/' . $value->pid . " class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";
                $action .= "</div'>";
                $first_action = "<a href=" . base_url() . 'admin/optometry/visits/' . $value->pid . ">";
