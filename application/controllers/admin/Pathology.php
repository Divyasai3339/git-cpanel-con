...
class Pathology extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        if (isset($total_rows) && !empty($total_rows)) {
            foreach ($total_rows as $row_key => $row_value) {
                ...
                if ($reference_range == "") {
                    $this->form_validation->set_rules('reference_range', $this->lang->line('reference_range'), 'trim|xss_clean');
                }
                if ($patho_unit == "") {
                ...
                if ($parameter_name == "") {
                    $this->form_validation->set_rules('parameter_name', $this->lang->line('test_parameter_name'), 'trim|xss_clean');
                } else {
        ...
        $this->form_validation->set_rules('code', $this->lang->line('charge_name'), 'xss_clean');
        $this->form_validation->set_rules('standard_charge', $this->lang->line('standard_charge'), 'xss_clean');
        $this->form_validation->set_rules('amount', $this->lang->line('amount'), 'xss_clean');
        $this->form_validation->set_rules('charge_category_id', $this->lang->line('charge_category'), 'xss_clean');
    ...
    public function getPatientPathologyDetails()
    {
        ...
        if (isset($is_bill)) {
           ...
           $actions .= "<a href='javascript:void(0)' onclick='myFunction()' data-loading-text='<i class=\"fa fa-circle-o-notch fa-spin\"></i>'  data-toggle='tooltip' data-placement='bottom'  data-record-id=\"" . $id . "\"   data-original-title='" . $this->lang->line('print_test_report') . "'><i class='fa fa-reorder'></i></a>";
    ...
    public function printtestparameterdetail()
    {
     ...
     public function getallselectedtest()
    {
        $print_details         = $this->printing_model->get('', 'pathology');
        $data['print_details'] = $print_details;
        $ids =$this->input->post('ids');
        $bill_id = $this->input->post('bill_id');
        $data['head_result']   = $this->pathology_model->getPathologyBillByID($bill_id);
        $result = [];
        foreach($ids as $id)
        {
            $result[]              = $this->pathology_model->getPatientPathologyReportDetails($id);
        ...
        $data['result'] = $result ;
        $page                  = $this->load->view('admin/pathology/_printselectedtestparameterdetail', $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));   
    }
