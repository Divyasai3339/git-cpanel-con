...
//====================================
    ...
    public function transactionreports()
    {
        ...
        $dt_data         = array();
        $total_amount    = 0;
        $total_cash=$total_cheque=$total_ttba=$total_upi=$total_other=$total_online=$total_card_pay = 0;

        if (!empty($transactiondata->data)) {
            foreach ($transactiondata->data as $key => $value) {
                if($value->amount < 0){
                  $total_refund += $value->amount;   
                }
                $total_amount += $value->amount;
                if($value->payment_mode=='Cash'){$total_cash += $value->amount;}
                if($value->payment_mode=='Cheque'){$total_cheque += $value->amount;}
                if($value->payment_mode=='transfer_to_bank_account'){$total_ttba += $value->amount;}
                if($value->payment_mode=='UPI'){$total_upi += $value->amount;}
                if($value->payment_mode=='Other'){$total_other += $value->amount;}
                if($value->payment_mode=='Online'){$total_online += $value->amount;}
                if($value->payment_mode=='card_payment'){$total_card_pay += $value->amount;}
            ...
            $footer_row   = array();
             $footer_row[] = "";
            //  "<b>Cash : </b>".number_format($total_cash, 2, '.', '');
            $footer_row[] = "";
            // "<b>Cheque : </b>".number_format($total_cheque, 2, '.', '');
            $footer_row[] = "";
            // "<b>Transfer to Bank Account : </b>".number_format($total_ttba, 2, '.', '');
            $footer_row[] = "";
            // "<b>UPI : </b>".number_format($total_upi, 2, '.', '');
            $footer_row[] = "";
            // "<b>Others : </b>".number_format($total_other, 2, '.', '');
            $footer_row[] = "";
            // "<b>Online : </b>".number_format($total_online, 2, '.', '');
            $footer_row[] = "";
            // "<b>Card Payment : </b>".number_format($total_card_pay, 2, '.', '');
            $footer_row[] = "<b>" . $this->lang->line('total_amount') . "</b>" . ':';
            $footer_row[] = "<b>" . (number_format($total_amount, 2, '.', '')) . "<br/><br/>"."<b>Cash : </b>".number_format($total_cash, 2, '.', ''). "<br/>"."<b>Cheque : </b>".number_format($total_cheque, 2, '.', '')."<br/>"."<b>Transfer to Bank Account : </b>".number_format($total_ttba, 2, '.', '')."<br/>"."<b>UPI : </b>".number_format($total_upi, 2, '.', '')."<br/>"."<b>Others : </b>".number_format($total_other, 2, '.', '')."<br/>"."<b>Online : </b>".number_format($total_online, 2, '.', '')."<br/>"."<b>Card Payment : </b>".number_format($total_card_pay, 2, '.', '')."<br/>"."<b>Refund : </b>".number_format($total_refund, 2, '.', '');
            $dt_data[]    = $footer_row;
        }
