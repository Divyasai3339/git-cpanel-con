...
class Issueitem extends Admin_Controller
{
    ...
    public function addconsumable()
    {
         ...
         $this->form_validation->set_rules('item_category_id_1', $this->lang->line('item_category'), 'required|trim|xss_clean');
         $this->form_validation->set_rules('item_id_1', $this->lang->line('item'), 'required|trim|xss_clean');
         $this->form_validation->set_rules('quantity_1', $this->lang->line('quantity'), 'trim|required|xss_clean|callback_check_available_quantity');
        $total_rows          = $this->input->post('rows');
        $item = 0;
        if (isset($total_rows) && !empty($total_rows)) {
            foreach ($total_rows as $row_key => $row_value) {
                $item_category = $this->input->post('item_category_id_' . $row_value);
                $item_name     = $this->input->post('item_id_' . $row_value);
                $qty            = $this->input->post('quantity_' . $row_value);
                ...
                if ($item_category !== "") {
                    ...
                    // if ($qty == "") {
                    //     $this->form_validation->set_rules('quantity', $this->lang->line('quantity'), 'trim|greater_than[0]|required|xss_clean');
                    // }
                }
            }
        }
        ...
        if ($this->form_validation->run() === FALSE) {
            $data = array(
                'item_category_id_1' => form_error('item_category_id_1'),
                'item_id_1'          => form_error('item_id_1'),
                'quantity_1'         => form_error('quantity_1'),
            ...
            if ($item == 1) {
                foreach ($total_rows as $row_key => $row_value) {
                        ...
                        $insert_items[] = array(
                            'item_category_id' => $this->input->post("item_category_id_" . $row_value),
            ...
            $data = array(
                ...
                'item_data'      => json_encode($insert_items),
            );
            
            $this->itemissue_model->add_consumable($data);
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
        echo json_encode($array);
    ...
    public function getConsumableBillDetails()
    {
        // if (!$this->rbac->hasPrivilege('pharmacy_bill', 'can_view')) {
        //     access_denied();
        // }
        $id  = $this->input->get('id');
        $detail = $this->itemissue_model->getConsumableBillDetails($id);
        $data['result'] = $detail['result_data'];
        $data['items'] =  $detail['item_array'];
        //echo "<pre>";
        // print_r($data);
        $action_details = "";
        ...
        $page = $this->load->view('admin/issueitem/_getConsumableBillDetails', $data, true);
        echo json_encode(array('status' => 1, 'page' => $page, 'actions' => $action_details));
    }

    public function deleteConsumableBill()
    {
        ...
        // }
        if (!empty($id)) {
            $this->itemissue_model->deleteConsumableBill($id);
