...
class medicalcamp_model extends MY_Model
{
    ...
    //             array_push($custom_field_column_array, 'table_custom_' . $i . '.field_value');
    //             array_push($field_var_array, '`table_custom_' . $i . '`.`field_value` as `' . $custom_fields_value->name.'`');
    //             $this->datatables->join('custom_field_values as '.$tb_counter,'medicalcamp_details.id = '.$tb_counter.'.belong_table_id AND '.$tb_counter.'.custom_field_id = '.$custom_fields_value->id,"left");
    //             $i++;
    //         }
    //     }
        ...
        // $custom_field_column = (empty($custom_field_column_array))? "": ",".implode(',', $custom_field_column_array);
        // $this->datatables
        // ->select('medicalcamp_details.id as mcampid,medicalcamp_details.case_reference_id,patients.id as pid,count(medicalcamp_details.patient_id) as total_visit,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.age,patients.gender,patients.mobileno,patients.is_ipd,staff.name,staff.surname,staff.employee_id'. $field_variable )
        // ->join('visit_details', "medicalcamp_details.id=visit_details.opd_details_id", "LEFT")
        // ->join('patients', "patients.id=medicalcamp_details.patient_id", "LEFT")
