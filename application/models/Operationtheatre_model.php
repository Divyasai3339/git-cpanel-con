...
class Operationtheatre_model extends MY_Model {
    ...
    public function getOtPatientDetails($otid) {
        ...
        $this->db->join('patients', 'operation_theatre.patient_id=patients.id', "inner");
        $this->db->join('staff', 'staff.id = operation_theatre.consultant_doctor', "inner");
        $this->db->join('iolmodel', 'iolmodel.id = operation_theatre.iol_model', "inner");
        $this->db->join('charges', 'operation_theatre.charge_id = charges.id');
    ...
    public function otdetails($ot_id) {
        ...
        $this->db->select('operation_theatre.*,operation.operation,operation_category.category,iolmodel.iolmodel_name,ioltype.ioltype_name,axial.axial_name,constant.constant_name,power.power_name,optin.optin_name,acpc.acpc_name,staff.name,staff.surname,staff.employee_id'.$field_variable);
        $this->db->join('staff', 'staff.id=operation_theatre.consultant_doctor');
        $this->db->join('iolmodel', 'iolmodel.id=operation_theatre.iol_model');
        $this->db->join('ioltype', 'ioltype.id=operation_theatre.iol_type');
        $this->db->join('axial', 'axial.id=operation_theatre.axial_length');
        $this->db->join('constant', 'constant.id=operation_theatre.a_constant');
        $this->db->join('power', 'power.id=operation_theatre.power');
        $this->db->join('optin', 'optin.id=operation_theatre.optin_dioptor');
        $this->db->join('acpc', 'acpc.id=operation_theatre.ac_pc');
        $this->db->join('operation', 'operation_theatre.operation_id=operation.id', "left");
        $this->db->join("operation_category","operation_category.id=operation.category_id","left");
    ...
    public function otdetailsforprint($ot_id,$patient_panel = null) {
        ...
        $this->db->select('operation_theatre.*,operation.operation,operation_category.category,iolmodel.iolmodel_name,ioltype.ioltype_name,axial.axial_name,constant.constant_name,power.power_name,optin.optin_name,acpc.acpc_name,staff.name,staff.surname,staff.employee_id,opd_details.patient_id as `opd_patient_id`,opd_patient.patient_name as `opd_patient_name`, ipd_patient.patient_name as `ipd_patient_name`,ipd_details.patient_id as `ipd_patient_id`,ipd_details.case_reference_id as ipd_case_id,opd_details.case_reference_id as opd_case_id'.$field_variable);
        $this->db->join('staff', 'staff.id=operation_theatre.consultant_doctor');
        $this->db->join('iolmodel', 'iolmodel.id=operation_theatre.iol_model');
        $this->db->join('ioltype', 'ioltype.id=operation_theatre.iol_type');
        $this->db->join('axial', 'axial.id=operation_theatre.axial_length');
    ...
    public function insertiolmodel($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function iolmodel_list($id = null)
    {
        $this->db->select()->from('iolmodel');
        if ($id != null) {
            $this->db->where('iolmodel.id', $id);
        } else {
            $this->db->order_by('iolmodel.iolmodel_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
     ...
     public function delete_iolmodel($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('iolmodel');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    // IOLTYPE START
      ...
      public function insertiol($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function ioltype_list($id = null)
    {
        $this->db->select()->from('ioltype');
        if ($id != null) {
            $this->db->where('ioltype.id', $id);
        } else {
            $this->db->order_by('ioltype.ioltype_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
     ...
     public function delete_ioltype($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('ioltype');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    public function insertaxial($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function axial_list($id = null)
    {
        $this->db->select()->from('axial');
        if ($id != null) {
            $this->db->where('axial.id', $id);
        } else {
            $this->db->order_by('axial.axial_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
     ...
     public function delete_axial($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('axial');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    public function insertconstant($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function constant_list($id = null)
    {
        $this->db->select()->from('constant');
        if ($id != null) {
            $this->db->where('constant.id', $id);
        } else {
            $this->db->order_by('constant.constant_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
     ...
     public function delete_constant($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('constant');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    public function insertpower($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function power_list($id = null)
    {
        $this->db->select()->from('power');
        if ($id != null) {
            $this->db->where('power.id', $id);
        } else {
            $this->db->order_by('power.power_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
     ...
     public function delete_power($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('power');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    // A optin
     ...
     public function insertoptin($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function optin_list($id = null)
    {
        $this->db->select()->from('optin');
        if ($id != null) {
            $this->db->where('optin.id', $id);
        } else {
            $this->db->order_by('optin.optin_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
    ...
    public function delete_optin($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id', $id);
        $this->db->delete('optin');        
        $message = DELETE_RECORD_CONSTANT . " On IOL Type " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
    ...
    // A acpc
     ...
     public function insertacpc($data,$tbl,$id=null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function acpc_list($id = null)
    {
        $this->db->select()->from('acpc');
        if ($id != null) {
            $this->db->where('acpc.id', $id);
        } else {
            $this->db->order_by('acpc.acpc_name');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();
    ...
    public function delete_acpc($id = null)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
     ...
     public function getipdoperationDetailsPrint($ipd_id) {

        $this->db->select('operation_theatre.*,operation.operation,operation_category.category,iolmodel.iolmodel_name,ioltype.ioltype_name,axial.axial_name,constant.constant_name,power.power_name,optin.optin_name,acpc.acpc_name,patients.id as pid,patients.patient_name,patients.gender,patients.age,patients.month,patients.patient_type,patients.mobileno,patients.is_active')->from('operation_theatre');
        $this->db->join('ipd_details', 'ipd_details.id=operation_theatre.ipd_details_id', "left");
        ...
        $this->db->join('optin', 'optin.id=operation_theatre.optin_dioptor');
        $this->db->join('acpc', 'acpc.id=operation_theatre.ac_pc');
        $this->db->where('operation_theatre.ipd_details_id', $ipd_id);
        $query = $this->db->get();
        return $query->result_array();
    }
}
