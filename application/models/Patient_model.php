...
class Patient_model extends MY_Model
{
    ...
    public function getstaffstoresbill()
    {
        $this->db->select('stores_bill_basic.generated_by,staff.id as staffid,staff.name as staffname, staff.surname as staffsurname,staff.employee_id');
        $this->db->join('staff', 'staff.id=stores_bill_basic.generated_by', 'left');
        $this->db->group_by('stores_bill_basic.generated_by');
    ...
    public function getAllopdRecord()
    {
       ...
       if (!empty($custom_fields)) {
        ...
        $this->datatables
            ...
            ->join('charges', "charges.id=patient_charges.charge_id", "LEFT")
    ...
    // public function getAllpatientStatusRecord()
    // {
    ...
    //     $custom_field_column = (empty($custom_field_column_array))? "": ",".implode(',', $custom_field_column_array);
    //     $this->datatables
    //         ->select('opd_details.id as opdid,appoint_priority.appoint_priority as priorityname,visit_details.appointment_date as report_time,visit_details.id as visis_id,opd_details.case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.name,staff.surname,

    //             optometry.ar_time as artime,optometry.optometrist_time as optime,optometry.dcot_time as doctime,
    //             optometry.antsegment_data as investigate_json,visit_details.dailation,visit_details.is_checkdout,
    //             visit_details.not_avialable,

    //             staff.employee_id,charges.name as charge_name'. $field_variable )
    //         ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
    //         ->join('patient_charges', "opd_details.id=patient_charges.opd_id", "LEFT")
    //         ->join('charges', "charges.id=patient_charges.charge_id", "LEFT")
    //         ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
    //         ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
    //         ->join('appoint_priority', 'appoint_priority.id = visit_details.priority_id', "LEFT")
    //         ->join('optometry', 'optometry.opd_id = opd_details.id', "LEFT")
    //         ->where('DATE(visit_details.appointment_date)',date('Y-m-d'))
    //         ->searchable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column)
    //         ->orderable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column.',MAX(visit_details.appointment_date)')
    //         ->sort('max(visit_details.appointment_date)', 'desc')
    //         ->group_by('patients.id')
    ...
    // }
    // public function updatePatientStatus($where,$data){
        
    //     $this->db->where($where);
    //     $result=$this->db->update('visit_details',$data);
    //     return $result;
    // }
  ...
  $this->db->select('visit_details.*,organisation.organisation_name,opd_details.id as opdid,opd_details.case_reference_id,opd_details.patient_id,patients.patient_name,patients.id as patient_id,patients.age,patients.month,patients.day,patients.dob,patients.guardian_name,patients.gender,patients.marital_status,patients.mobileno,patients.email,patients.address,patients.insurance_id,patients.insurance_validity,patients.identification_number,patients.known_allergies,patients.image as patient_image,blood_bank_products.name as blood_group_name,staff.name,staff.surname,staff.employee_id,patients.id as `patient_id`')->from('visit_details');
    ...
    public function getAllopdrechekupRecord($patientid, $opdid)
    {
        ...
        $this->datatables
            ->select('opd_details.*,patients.id as pid,patients.is_ipd,patients.organisation,patients.old_patient,patients.patient_name,patients.biohazard,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,consult_charges.standard_charge,patient_charges.apply_charge')
            ->join('staff', 'staff.id = opd_details.cons_doctor', "inner")
            ->join('patients', 'patients.id = opd_details.patient_id', "LEFT")
    ...
    public function getLatestopdvisitRecord()
    {
        //$date = date("Y-m-d");
        $setting            = $this->setting_model->get();
        $opd_month          = $setting[0]['opd_record_month'];
       ...
       if (!empty($custom_fields)) {
        ...
        $doctor ="";
         ...
         $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.image,patients.mobileno,patients.is_ipd,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,
                optom.dcot_time,optom.surgery_recommendations,optom.ar_time,optom.optometrist_time,
                charges.name as charge_name'. $field_variable )
            ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
            ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
            ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
            ->join('patient_charges', "opd_details.id=patient_charges.opd_id", "LEFT")
            ->join('charges', "charges.id=patient_charges.charge_id", "LEFT")
            ->join('optometry optom', "optom.opd_id=opd_details.id", "LEFT")
            ->join('ipd_prescription_basic', "visit_details.id = ipd_prescription_basic.visit_details_id", "LEFT")
            ->where('DATE(visit_details.appointment_date)',date('Y-m-d'))
            ->where('opd_details.discharged', 'no')
            ->searchable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column)
            ->orderable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column.',MAX(visit_details.appointment_date)')
            ...
            ->group_by('patients.id')
            ->from('opd_details');
            if(isset($sessiondata['hospitaladmin']['roles']) && $sessiondata['hospitaladmin']['roles']['Doctor']==3){
                $doctor = $sessiondata['hospitaladmin']['id']; 
                $this->datatables->where('visit_details.cons_doctor',$doctor);    
             }

         /* Old Query
        $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.name,staff.surname,staff.employee_id'. $field_variable )
            ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
            ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
            ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
            ->where('DATE(visit_details.appointment_date)',date('Y-m-d'))
    ...
    public function getallconsumable($ipdid){
       
           $query = $this->db->query("SELECT item_consumable.*, patients.patient_name, ipd_details.case_reference_id, staff.name AS doctor
           FROM item_consumable
           LEFT JOIN patients ON patients.id = item_consumable.patient_id
           LEFT JOIN ipd_details ON ipd_details.id = item_consumable.ipd_id  
           LEFT JOIN staff ON staff.id = ipd_details.cons_doctor 
           WHERE item_consumable.ipd_id=".$ipdid." order by id desc");
           $result = $query->result_array();
          return $result ;
    }
    ...
    {
        $query = $this->db->select("opd_details.id,opd_details.case_reference_id,visit_details.id as visit_id,visit_details.symptoms,visit_details.appointment_date, ipd_prescription_basic.finding_description,
        ipd_prescription_basic.complaints_description, patients.patient_name,patients.dob,patients.age,patients.month,patients.day,patients.gender,patients.mobileno,patients.guardian_name,patients.address,patients.id patientid,staff.name,staff.surname,staff.employee_id")
            ->join('visit_details', 'opd_details.id = visit_details.opd_details_id', "left")
    ...
    public function getPatientDataByVisitID($visit_detail_id){
        $this->db
        ->select('visit_details.id,opd_details.id,patients.patient_name,patients.gender,patients.dob,patients.age')
        ->join('opd_details', 'opd_details.id = visit_details.opd_details_id', "left")
        ->join('patients', 'patients.id = opd_details.patient_id', "left")
        ->where('visit_details.id', $visit_detail_id)
        ->from('visit_details');
        $query = $this->db->get();
        $row = $query->row_array();
        return $row ;
    ...
    public function getSurgeriesOpdVisitRecord()
    {
        //$date = date("Y-m-d");
        $setting            = $this->setting_model->get();
        $opd_month          = $setting[0]['opd_record_month'];
       ...
       if (!empty($custom_fields)) {
        ...
        $doctor ="";
         ...
         $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,optom.surgery_recommendations as sg,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.id as staff_id,staff.name,staff.surname,staff.employee_id'. $field_variable )
            ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
            ->join('optometry optom', "optom.opd_id=opd_details.id", "LEFT")
            ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
            ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
    ...
    public function add_surgery($data)
    {
        ...
        if (isset($data['id']) && $data['id'] != '') {
            $this->db->where('id', $data['id']);
            $this->db->update('sugeries', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On Surgery id " . $data['id'];
            $action    = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('sugeries', $data);
           
            $insert_id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On Surgery id " . $insert_id;
            $action    = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
    ...
    public function GetLastVisitDetailByPatientID($patient_id){
        
        $this->db
        ->select('opd_details.*,optometry.vision_data,optometry.dryretinoscopy_data,optometry.cyclo_data,optometry.acceptance_data,optometry.pgp_data,optometry.diagnosis_data')
        ->join('optometry', 'opd_details.id = optometry.opd_id', "left")
        ->where('opd_details.patient_id', $patient_id)
    ...
    public function GetPatientDetail($patient_id){
        ...
        $query = $this->db->get_where('patients',array("id"=>$patient_id));
        return $query->row_array();
    }
    
    // checkout
    
    public function getLatestcheckoutRecord()
    {
        //$date = date("Y-m-d");
        $setting            = $this->setting_model->get();
        $opd_month          = $setting[0]['opd_record_month'];
       ...
       if (!empty($custom_fields)) {
        ...
        $doctor ="";
         ...
         $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,surgeries.followup_date,surgeries.status,surgeries.surgery_date,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,ipd_prescription_basic.surgery_recommendations'. $field_variable )
            ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
            ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
            ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
            ->join('optometry', 'opd_details.id = optometry.opd_id', "LEFT")
             ->join('surgeries', 'opd_details.id = surgeries.opd_id', "LEFT")
            ->join('ipd_prescription_basic', "visit_details.id = ipd_prescription_basic.visit_details_id", "LEFT")
            ->where('DATE(visit_details.appointment_date)',date('Y-m-d'))
            ->where('opd_details.discharged', 'yes')
            ->searchable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column)
    ...
    public function opdMoveToDischarge($opdids)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
            $this->db->where_in('id', $opdids);
            $this->db->update('opd_details', array("discharged"=>"yes"));
            $message   = UPDATE_RECORD_CONSTANT . " On Opd Details " . end($opdids);
            $action    = "Update";
            $record_id = end($opdids);
            $this->log($message, $record_id, $action);
            //======================Code End==============================
        ...
        public function getAllcampRecord()
    {
        $this->datatables->select('*')
        ->sort('medicalcamp_details.id', 'desc')
        ->from('medicalcamp_details');
        return $this->datatables->generate('json');
    } 
    
        public function add_patientcamp($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data['id']) && $data['id'] != '') {
            $this->db->where('id', $data['id']);
            $this->db->update('medicalcamp_details', $data);
            $message   = UPDATE_RECORD_CONSTANT . " On medicalcamp_details id " . $data['id'];
            $action    = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            ...
            $this->db->insert('medicalcamp_details', $data);
        //    return $this->db->last_Query();
            $insert_id = $this->db->insert_id();
            $message   = INSERT_RECORD_CONSTANT . " On medicalcamp_details id " . $insert_id;
            $action    = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
            //======================Code End==============================
        ...
        public function getMedicalCamppatientbyid($id)
    {
        $this->db->select('medicalcamp_details.*')->from('medicalcamp_details')->where('medicalcamp_details.id', $id);
        $query = $this->db->get();
     
        return $query->row_array();
    }
    
     public function getIpdDetailsdischarge($ipd_id)
    {
        $i = 1;
        $custom_fields   = $this->customfield_model->get_custom_fields('patient','','','', 1);
        $custom_field_column_array= array();
    ...
    //  public function counsellinggetLatestopdvisitRecord()
    // {
    //     //$date = date("Y-m-d");
    //     $setting            = $this->setting_model->get();
    //     $opd_month          = $setting[0]['opd_record_month'];
    ...
    //         ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,
    //             optom.dcot_time,optom.surgery_recommendations,optom.ar_time,optom.optometrist_time,
    //             sugeries.followup_date,sugeries.status,sugeries.surgery_date,
    //             charges.name as charge_name'. $field_variable )
    //         ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
    //         ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
    //         ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
    ...
    //         ->join('charges', "charges.id=patient_charges.charge_id", "LEFT")
    //         ->join('optometry optom', "optom.opd_id=opd_details.id", "LEFT")
    //         ->join('sugeries', 'opd_details.id = sugeries.opd_id', "LEFT")
    //         ->join('ipd_prescription_basic', "visit_details.id = ipd_prescription_basic.visit_details_id", "LEFT")
    //         // ->where('DATE(visit_details.appointment_date)',date('Y-m-d'))
    //         ->where('opd_details.discharged', 'no')
    //         ->searchable('patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name'. $custom_field_column)
    ...
    // }
     ...
     public function counsellinggetLatestopdvisitRecord($filters=NULL)
    {
        //$date = date("Y-m-d");
        $setting            = $this->setting_model->get();
        $opd_month          = $setting[0]['opd_record_month'];
       ...
       if (!empty($custom_fields)) {
        ...
        {
           $start_date=(isset($filters['date_from']) && $filters['date_from']!='')?date('Y-m-d',strtotime($filters['date_from'])):NULL;
           $end_date=(isset($filters['date_to']) && $filters['date_to']!='')?date('Y-m-d',strtotime($filters['date_to'])):NULL;

           $status=(isset($filters['status']) && $filters['status']!='')?$filters['status']:NULL;
           $insure=(isset($filters['insure']) && $filters['insure']!='')?$filters['insure']:NULL;
           
           if($start_date!=NULL && $end_date)
           {
             $this->datatables->where('DATE_FORMAT(sugeries.surgery_date,"%Y-%m-%d") >=', $start_date);
             $this->datatables->where('DATE_FORMAT(sugeries.surgery_date,"%Y-%m-%d") <=', $end_date);
             //$this->datatables->where('sugeries.surgery_date',date('Y-m-d',strtotime($duration)));
           }
           if($status!=NULL)
           {
             $this->datatables->where('sugeries.status',$status);    
           }
           if($insure!=NULL)
           {
             $this->datatables->where('sugeries.insurer',$insure);    
           }
        }
         $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,
                optom.dcot_time,optom.surgery_recommendations,optom.ar_time,optom.optometrist_time,
                sugeries.followup_date,sugeries.status,sugeries.surgery_date,sugeries.id as surgery_id,
                charges.name as charge_name'. $field_variable )
            ->join('visit_details', "opd_details.id=visit_details.opd_details_id", "LEFT")
            ->join('patients', "patients.id=opd_details.patient_id", "LEFT")
            ->join('staff', 'staff.id = visit_details.cons_doctor', "LEFT")
     ...
     public function GetoptocounDetail($patient_id){
        $query = $this->db->get_where('optometry',array("patient_id"=>$patient_id));
        return $query->row_array();
    }
    
    public function GetvisitDetail($opd){
        $query = $this->db->get_where('opd_details',array("id"=>$opd));
        return $query->row_array();
    }
    
    public function Getvisitall($opd){
        // $query = $this->db->get_where('visit_details',array("opd_details_id"=>$opd));
        
        // return $query->row_array();
        
         $this->db->select('visit_details.*, staff.name');
        $this->db->from('visit_details');
        $this->db->join('staff', 'staff.id = visit_details.cons_doctor', 'left');  // Assuming a relationship based on 'staff_id'
        $this->db->where('opd_details_id', $opd);
    ...
    return $query->row_array();
     ...
     public function recordfilter($insurer = null, $date= null, $status)
    {
         $this->datatables
            ->select('max(opd_details.id) as opdid,max(opd_details.case_reference_id) as case_reference_id,patients.id as pid,count(opd_details.patient_id) as total_visit,max(visit_details.id) as visit_id,max(visit_details.appointment_date) as last_visit,patients.patient_name,patients.id as patientid,patients.guardian_name,patients.gender,patients.mobileno,patients.is_ipd,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,
                optom.dcot_time,optom.surgery_recommendations,optom.ar_time,optom.optometrist_time,
    ...
    public function add_prep($data)
    {
        if (isset($data['id']) && $data['id'] != '') {
            $this->db->where('id', $data['id']);
            $this->db->update('discharge_prep', $data);
        } else {
            $this->db->insert('discharge_prep', $data);
            return $this->db->insert_id();
        }

    }

    public function get_prep($card_data_prep)
    {
        foreach ($card_data_prep as $key => $value) {
            $this->db->where($key, $value);
        }
        
        $card = $this->db->select('*')->from('discharge_prep')->get()->row_array();
        return $card;

    }

    public function getIpdDetailsprep($ipd_id)
    {
        $i = 1;
        $custom_fields   = $this->customfield_model->get_custom_fields('patient','','','', 1);
        $custom_field_column_array= array();
        ...
        $this->db->select('patients.*,blood_bank_products.name as blood_group_name,ipd_details.patient_old,ipd_details.id as ipdid,ipd_details.patient_id,discharge_prep.discharge_date,ipd_details.date,ipd_details.date,ipd_details.case_type,ipd_details.id as ipdid,ipd_details.casualty,ipd_details.height,ipd_details.weight,ipd_details.organisation_id,ipd_details.bp,ipd_details.cons_doctor,ipd_details.refference,ipd_details.known_allergies as ipdknown_allergies,ipd_details.case_reference_id,ipd_details.credit_limit as ipdcredit_limit,ipd_details.symptoms,ipd_details.discharged as ipd_discharge,ipd_details.bed,ipd_details.bed_group_id,ipd_details.note as ipdnote,ipd_details.bed,ipd_details.bed_group_id,ipd_details.payment_mode,ipd_details.credit_limit,ipd_details.pulse,ipd_details.temperature,ipd_details.respiration,ipd_details.   organisation_id,staff.id as staff_id,staff.name,staff.surname,staff.image as doctor_image,staff.employee_id,bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name'.$field_variable)->from('ipd_details');
        $this->db->join('patients', 'patients.id = ipd_details.patient_id', "left");
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id','left');
        $this->db->join('discharge_prep', 'ipd_details.id = discharge_prep.ipd_details_id', "left");
        $this->db->join('staff', 'staff.id = ipd_details.cons_doctor', "inner");
        $this->db->join('organisation', 'organisation.id = ipd_details.organisation_id', "left");
        $this->db->join('bed', 'ipd_details.bed = bed.id', "left");
        $this->db->join('bed_group', 'ipd_details.bed_group_id = bed_group.id', "left");
    ...
    public function add_consent($data)
    {
        if (isset($data['id']) && $data['id'] != '') {
            $this->db->where('id', $data['id']);
            $this->db->update('discharge_consent', $data);
        } else {
            $this->db->insert('discharge_consent', $data);
            return $this->db->insert_id();
        }

    }

    public function get_consent($card_data_consent)
    {
        foreach ($card_data_consent as $key => $value) {
            $this->db->where($key, $value);
        }
        
        $card = $this->db->select('*')->from('discharge_consent')->get()->row_array();
        return $card;

    }

    public function getIpdDetailsconsent($ipd_id)
    {
        $i = 1;
        $custom_fields   = $this->customfield_model->get_custom_fields('patient','','','', 1);
        $custom_field_column_array= array();
        ...
        $this->db->select('patients.*,blood_bank_products.name as blood_group_name,ipd_details.patient_old,ipd_details.id as ipdid,ipd_details.patient_id,discharge_consent.surgery_date,discharge_consent.surgeon_name,discharge_consent.kmc_no,ipd_details.date,ipd_details.date,ipd_details.case_type,ipd_details.id as ipdid,ipd_details.casualty,ipd_details.height,ipd_details.weight,ipd_details.organisation_id,ipd_details.bp,ipd_details.cons_doctor,ipd_details.refference,ipd_details.known_allergies as ipdknown_allergies,ipd_details.case_reference_id,ipd_details.credit_limit as ipdcredit_limit,ipd_details.symptoms,ipd_details.discharged as ipd_discharge,ipd_details.bed,ipd_details.bed_group_id,ipd_details.note as ipdnote,ipd_details.bed,ipd_details.bed_group_id,ipd_details.payment_mode,ipd_details.credit_limit,ipd_details.pulse,ipd_details.temperature,ipd_details.respiration,ipd_details.   organisation_id,staff.id as staff_id,staff.name,staff.surname,staff.image as doctor_image,staff.employee_id,bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name'.$field_variable)->from('ipd_details');
        $this->db->join('patients', 'patients.id = ipd_details.patient_id', "left");
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id','left');
        $this->db->join('discharge_consent', 'ipd_details.id = discharge_consent.ipd_details_id', "left");
        $this->db->join('staff', 'staff.id = ipd_details.cons_doctor', "inner");
        $this->db->join('organisation', 'organisation.id = ipd_details.organisation_id', "left");
        $this->db->join('bed', 'ipd_details.bed = bed.id', "left");
        $this->db->join('bed_group', 'ipd_details.bed_group_id = bed_group.id', "left");
