...
{   
    ...
    {   
        ...
        $item_query = $this->db->get('item_stock');
        $get_item = $item_query->row_array();
        if (isset($get_item['id']) && $get_item['id'] != '') {
            $data['quantity'] = $get_item['quantity'] + $data['quantity'];
            $this->db->where('id', $get_item['id']);
            $this->db->update('item_stock', $data);
            $message = UPDATE_RECORD_CONSTANT . " For Item Stock id " . $data['id'];
            $action = "Update";
            $record_id = $get_item['id'];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
            $this->db->trans_complete(); # Completing transaction
            /* Optional */
