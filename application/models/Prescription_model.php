...
class Prescription_model extends MY_Model
{
    ...
    public function getMedTemplates(){
        $this->db->where('master_type','medicine_templates');
        $query = $this->db->get('master_data');
        return $query->result_array();

    }

    public function add_template_prescription($insert_medicines)
    {
        $count = count($insert_medicines);
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 0s1. If you wish can remove as well
        $this->db->insert_batch("med_template_details", $insert_medicines);
        $this->db->trans_commit();
        $first_id = $this->db->insert_id();
        return $first_id + ($count-1);
       
    }

    public function is_medicine_exist($template_id){

        $query = $this->db->get_where("med_template_details",array("template_id"=>$template_id));
        return $query->num_rows();
    }
    ...
    public function getPrescriptionByTable($id, $table_type)
    {
        if ($table_type == "ipd_prescription") {
            ...
            $query = $this->db->select("opd_details.*,patients.*,staff.name,staff.surname,staff.local_address,ipd_prescription_basic.ipd_id,ipd_prescription_basic.complaints,ipd_prescription_basic.id as prescription_id,ipd_prescription_basic.date as presdate,ipd_prescription_basic.header_note,ipd_prescription_basic.footer_note,ipd_prescription_basic.finding_description,ipd_prescription_basic.complaints_description,ipd_prescription_basic.is_complaints_print,ipd_prescription_basic.is_finding_print,visit_details.id as visit_details_id,,ipd_prescription_basic.examinations,ipd_prescription_basic.diagnosis,ipd_prescription_basic.opd_procedure,ipd_prescription_basic.surgery_recommendations");
    ...
    public function getTemplatePrescriptionMedicinesByTemplateID($id)
    {
        $query = $this->db->select('pharmacy.id as pharmacy_id,pharmacy.medicine_name,medicine_category.id as medicine_category_id,medicine_category.medicine_category,med_template_details.instruction,med_template_details.dose_interval_id,med_template_details.dose_duration_id,dose_duration.name as dose_duration_name,dose_interval.name as dose_interval_name,medicine_dosage.dosage,charge_units.unit,med_template_details.dosage as dosage_id,med_template_details.id as template_id')
            ->join("pharmacy", "med_template_details.medicine_id = pharmacy.id")
            ->join("medicine_category", "medicine_category.id=pharmacy.medicine_category_id")
            ->join("medicine_dosage", "medicine_dosage.id=med_template_details.dosage","left")
            ->join("charge_units", "charge_units.id=medicine_dosage.charge_units_id","left")
            ->join("dose_interval", "dose_interval.id=med_template_details.dose_interval_id",'left')
            ->join("dose_duration", "dose_duration.id=med_template_details.dose_duration_id",'left')
            ->where("med_template_details.template_id", $id)
            ->get("med_template_details");
    ...
    public function getTemplateNameByID($template_id){
        $query = $this->db->get_where("master_data",array("id"=>$template_id));
        $data = $query->row_array();
        return $data['master_value'];
    }
    ...
    {
        $query = $this->db->select("opd_details.*,finding_category.category as fcat, complaint_category.category as ccat,visit_details.id as visitid,visit_details.known_allergies as any_allergies,visit_details.weight,visit_details.height,visit_details.pulse,visit_details.temperature,visit_details.symptoms,visit_details.bp,patients.*,blood_bank_products.name as blood_group_name,staff.name,staff.surname,staff.employee_id,staff.local_address,ipd_prescription_basic.ipd_id,ipd_prescription_basic.id as prescription_id,ipd_prescription_basic.date as presdate,ipd_prescription_basic.header_note,ipd_prescription_basic.footer_note,ipd_prescription_basic.finding_description,ipd_prescription_basic.complaints_description,ipd_prescription_basic.is_finding_print,ipd_prescription_basic.is_complaints_print,prescription_generate.name as generated_by_name,prescription_generate.surname as generated_by_surname,prescription_generate.employee_id as generated_by_employee_id,prescribe_by.name as prescribe_by_name,prescribe_by.surname as prescribe_by_surname,prescribe_by.employee_id as prescribe_by_employee_id, opd_details.id as opd_detail_id,staff.employee_id as doctor_id,ipd_prescription_basic.examinations,ipd_prescription_basic.diagnosis,ipd_prescription_basic.opd_procedure,ipd_prescription_basic.surgery_recommendations,ipd_prescription_basic.complaints,ipd_prescription_basic.history,ipd_prescription_basic.complaints");
        ...
        $this->db->join('finding_category', 'finding_category.id = ipd_prescription_basic.finding_categoryid', "left");
        $this->db->join('complaint_category', 'complaint_category.id = ipd_prescription_basic.complaint_categoryid', "left");
