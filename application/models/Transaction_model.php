...
{    
    ...
    public function storesPaymentByTransactionId($transaction_id)
    {
        $query = $this->db->select('transactions.*,stores_bill_basic.id as stores_bill_basic_id,patients.note as pnote,patients.id as patient_id,patients.patient_name,patients.guardian_name,patients.gender,patients.id as patient_unique_id,patients.mobileno,patients.email,patients.dob,patients.image,patients.address')
            ->join("stores_bill_basic", "stores_bill_basic.id = transactions.stores_bill_basic_id")
   ...
   public function ipdopdPaymentByCaseId($case_id)
    ...
    public function storesPayments($stores_bill_basic_id)
    {
        $query = $this->db->select('transactions.*,stores_bill_basic.id as stores_bill_basic_id,patients.note as pnote')
            ->join("stores_bill_basic", "stores_bill_basic.id = transactions.stores_bill_basic_id")
            ->join("patients", "patients.id = stores_bill_basic.patient_id")
            ->where("transactions.stores_bill_basic_id", $stores_bill_basic_id)
            ->order_by("transactions.payment_date", "desc")
            ->get("transactions");
        return $query->result();
    }
    ...
    public function storesTotalPayments($stores_bill_basic_id)
    {
        
        $query = $this->db->select('IFNULL((select sum(amount) as amount_paid from transactions WHERE transactions.stores_bill_basic_id =stores_bill_basic.id and transactions.type="payment" ),0) as total_paid, IFNULL((select sum(amount) as refund from transactions WHERE transactions.stores_bill_basic_id =stores_bill_basic.id and transactions.type="refund" ),0) as refund_amount, stores_bill_basic.*')
                 
            ->join("patients", "patients.id = stores_bill_basic.patient_id")
            ->where("stores_bill_basic.id", $stores_bill_basic_id)
    ...
    public function ipdpatientreportsRecord($data) {
        ...
        $sql="select ipd_details.id,'ipd_no' module_no,ipd_details.symptoms,organisation.organisation_name,optometry.surgery_recommendations,ipd_details.date, ipd_prescription_basic.finding_description, patients.patient_name,patients.dob,patients.age,patients.month,patients.day,patients.gender,patients.mobileno,patients.guardian_name,patients.address,patients.id patientid,staff.name,staff.surname,staff.employee_id ".$field_variable." from ipd_details 
        left join ipd_prescription_basic on ipd_prescription_basic.ipd_id = ipd_details.id  
        left join patients on patients.id = ipd_details.patient_id
        left join optometry on optometry.opd_id = ipd_details.case_reference_id
        left join organisation on organisation.id = ipd_details.organisation_id
    ...
    public function storesbillreportsRecord($start_date, $end_date,$searchdata=null) {
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                ...
                array_push($custom_field_column_array, 'table_custom_' . $i . '.field_value');
                array_push($field_var_array, '`table_custom_' . $i . '`.`field_value` as `' . $custom_fields_value->name.'`');
                $custom_join .= ('LEFT JOIN custom_field_values as '.$tb_counter.' ON transactions.stores_bill_basic_id = '.$tb_counter.'.belong_table_id AND '.$tb_counter.'.custom_field_id = '.$custom_fields_value->id." ");
                $i++;
            }
        }
        ...
        if($searchdata['collect_staff']!=""){
            $condition = " and stores_bill_basic.generated_by=".$searchdata['collect_staff'] ;
        }
       ...
       $sql="SELECT `stores_bill_basic`.*,ipd_prescription_basic.ipd_id,ipd_prescription_basic.visit_details_id, IFNULL((select sum(amount) as amount_paid from transactions WHERE transactions.stores_bill_basic_id =stores_bill_basic.id and transactions.type='payment' ".$condition_payment_mode." ), 0) as paid_amount, IFNULL((select sum(amount) as refund from transactions WHERE transactions.stores_bill_basic_id =stores_bill_basic.id and transactions.type='refund' ), 0) as refund_amount, `patients`.`patient_name`, `patients`.`id` as `pid`,patients.gender, patients.age,patients.month,patients.day,staff.name,staff.surname,staff.employee_id ".$field_variable." FROM `stores_bill_basic` LEFT JOIN `patients` ON `patients`.`id` = `stores_bill_basic`.`patient_id`  LEFT JOIN `ipd_prescription_basic` ON `ipd_prescription_basic`.`id` = `stores_bill_basic`.`ipd_prescription_basic_id` LEFT JOIN staff on staff.id = stores_bill_basic.generated_by  ".$custom_join." where DATE(stores_bill_basic.date) >='". $start_date."' and DATE(stores_bill_basic.date) <= '".$end_date."' ".$condition."";
             $this->datatables->query($sql) 
              ->searchable('stores_bill_basic.id,patients.patient_name'.$custom_field_column)
              ->orderable('stores_bill_basic.id,stores_bill_basic.date,patients.patient_name,patients.age,patients.gender,stores_bill_basic.ipd_prescription_basic_id,stores_bill_basic.doctor_name,staff.name'.$custom_field_column.',stores_bill_basic.net_amount,paid_amount')
              ->sort('stores_bill_basic.date','desc')
              ->query_where_enable(TRUE);
...
//new function
    ...
    public function storespaymentbybillid($bill_id,$patient_id)
    {
        $query = $this->db->select('transactions.*,stores_bill_basic.id as stores_bill_basic_id,patients.note as pnote,patients.id as patient_id,patients.patient_name,patients.guardian_name,patients.gender,patients.id as patient_unique_id,patients.mobileno,patients.email,patients.dob,patients.image,patients.address')
            ->join("stores_bill_basic", "stores_bill_basic.id = transactions.stores_bill_basic_id")
            ->join("patients", "patients.id = stores_bill_basic.patient_id")
            ->where("stores_bill_basic_id", $bill_id)
             ->where("transactions.patient_id", $patient_id)
            ->order_by("transactions.id", "desc")
            ->get("transactions");
        return $query->result_array();
    ...
    public function getopdpaymentdetails()
    {
      ...
      public function get_todayTransaction($where)
    {
        $result=$this->db->select('sum(amount) as total')->from('transactions');
     ...
     public function get_today_Refund_Transaction($where)
    {
        $result=$this->db->select('sum(amount) as total')->from('transactions');
        $this->db->where('amount < 0 ');
        $this->db->where($where);
        ...
        public function get_todaypharmacyTransaction($where)
    {
        $result=$this->db->select('sum(amount) as total')->from('transactions');
        $this->db->where('amount > 0 ');
        $this->db->where($where);
        $this->db->like('payment_date',  date("Y-m-d"));
        $result=$this->db->get()->row_array();
        // if($result['total']>0){
        //     return $result['total'];
        // }else{
        ...
        public function get_todayTransactionLike($where)
    {
         $result=$this->db->select('sum(amount) as total')->from('transactions');
        $this->db->where('amount > 0 ');
        $this->db->where($where);
