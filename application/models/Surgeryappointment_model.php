...
class Surgeryappointment_model extends MY_Model
{
    public function getShiftdata($doctor, $day, $shift,$ot,$patient)
    {
        ...
        $this->db->where("ot_no", $ot);*/
        //$this->db->where("global_shift_id", $shift);
        $this->db->where("day", $day);
        $query  = $this->db->get("doctor_shift_1");
        $result = $query->result();
        return $result;
    }
    ...
    public function getAppointments($ot, $shift_id, $date)
    {
        $this->db->select("time");
        //$this->db->where("doctor", $doctor_id);
        $this->db->where("ot_number", $ot);
        //$this->db->where("status", "Pending");
        $this->db->where("date_format(surgery_date,'%Y-%m-%d')", $date);
        $query         = $this->db->get("sugeries");
