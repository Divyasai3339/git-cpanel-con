...
class Pharmacy_model extends MY_Model
{
    ...
    public function getAllpharmacypurchaseRecords()
    {
        $this->datatables
            ->select('supplier_bill_basic.*,medicine_supplier.supplier')
            ->join('medicine_supplier', 'medicine_supplier.id = supplier_bill_basic.supplier_id')
            ->searchable('supplier_bill_basic.id,supplier_bill_basic.invoice_no,supplier')
            ->orderable('supplier_bill_basic.id,supplier_bill_basic.date,supplier_bill_basic.invoice_no,supplier,supplier_bill_basic.total,supplier_bill_basic.tax,supplier_bill_basic.discount,supplier_bill_basic.net_amount')
            ->where('supplier_bill_basic.supplier_id', 18)
            ->sort('supplier_bill_basic.id', 'desc')
            ->from('supplier_bill_basic');
        return $this->datatables->generate('json');
    }
    ...
    public function getBatchNumberByBatch($batch){
        ...
        if(!empty($batch)){
             $this->db->select('medicine_batch_details.batch_no,medicine_batch_details.id');
    ...
    public function getIPPharmacy($id = null)
    {
        $query = $this->db->get('ip_pharmacy');
        return $query->result_array();
    }
    
    public function getAllIPpharmacypurchaseRecord()
    {
        $this->datatables
            ->select('ip_supplier_bill_basic.*,medicine_supplier.supplier')
            ->join('medicine_supplier', 'medicine_supplier.id = ip_supplier_bill_basic.supplier_id')
            ->searchable('ip_supplier_bill_basic.id,ip_supplier_bill_basic.invoice_no,supplier')
            ->orderable('ip_supplier_bill_basic.id,ip_supplier_bill_basic.date,ip_supplier_bill_basic.invoice_no,supplier,ip_supplier_bill_basic.total,ip_supplier_bill_basic.tax,ip_supplier_bill_basic.discount,ip_supplier_bill_basic.net_amount')
            ->sort('ip_supplier_bill_basic.id', 'desc')
            ->from('ip_supplier_bill_basic');
        return $this->datatables->generate('json');
    }
    
    public function getAllIPpharmacypurchaseRecords()
    {
        $this->datatables
            ->select('ip_supplier_bill_basic.*,medicine_supplier.supplier')
            ->join('medicine_supplier', 'medicine_supplier.id = ip_supplier_bill_basic.supplier_id')
            ->searchable('ip_supplier_bill_basic.id,ip_supplier_bill_basic.invoice_no,supplier')
            ->orderable('ip_supplier_bill_basic.id,ip_supplier_bill_basic.date,ip_supplier_bill_basic.invoice_no,supplier,ip_supplier_bill_basic.total,ip_supplier_bill_basic.tax,ip_supplier_bill_basic.discount,ip_supplier_bill_basic.net_amount')
            ->where('ip_supplier_bill_basic.supplier_id', 18)
            ->sort('ip_supplier_bill_basic.id', 'desc')
            ->from('ip_supplier_bill_basic');
        return $this->datatables->generate('json');
    }
    
    public function addIPBillSupplier($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data["id"])) {
            $this->db->where("id", $data["id"])->update("ip_supplier_bill_basic", $data);
            $message = UPDATE_RECORD_CONSTANT . " On Ip Supplier Bill Basic id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert("ip_supplier_bill_basic", $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Ip Supplier Bill Basic id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
    ...
    public function addIPBillMedicineBatchSupplier($data1)
    {
        $query = $this->db->insert_batch('ip_medicine_batch_details', $data1);
    }
    
    public function searchIPFullText()
    {
        $this->db->select('ip_pharmacy.*,medicine_category.id as medicine_category_id,medicine_category.medicine_category');
        $this->db->join('medicine_category', 'ip_pharmacy.medicine_category_id = medicine_category.id', 'left');
        $this->db->where('`ip_pharmacy`.`medicine_category_id`=`medicine_category`.`id`');
        $this->db->order_by('ip_pharmacy.medicine_name');
        $query = $this->db->get('ip_pharmacy');
        return $query->result_array();
    }
    
    public function totalIPQuantity($pharmacy_id)
    {
        $query = $this->db->select('sum(available_quantity) as total_qty')
            ->where('pharmacy_id', $pharmacy_id)
            ->get('ip_medicine_batch_details');
        return $query->row_array();
    }
    
    public function add_ip($pharmacy)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->insert('ip_pharmacy', $pharmacy);
        $insert_id = $this->db->insert_id();
        $message = INSERT_RECORD_CONSTANT . " On Ip Pharmacy id " . $insert_id;
        $action = "Insert";
        $record_id = $insert_id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
    ...
    public function getAllIPpharmacyRecord()
    {
        $this->datatables
            ->select('ip_pharmacy.*,medicine_category.id as medicine_categoryid,medicine_category.medicine_category,(SELECT sum(available_quantity) FROM `ip_medicine_batch_details` WHERE pharmacy_id=ip_pharmacy.id) as `total_qty`')
            ->join('medicine_category', 'ip_pharmacy.medicine_category_id = medicine_category.id', 'left')
            ->join('ip_medicine_batch_details', 'ip_pharmacy.id = ip_medicine_batch_details.pharmacy_id', 'left')
            ->join('ip_pharmacy_bill_detail', 'ip_pharmacy_bill_detail.medicine_batch_detail_id = ip_medicine_batch_details.id', 'left')
            ->searchable('ip_pharmacy.medicine_name,ip_pharmacy.medicine_company,ip_pharmacy. medicine_composition,ip_pharmacy.medicine_category_id,ip_pharmacy.medicine_group')
            ->orderable('ip_pharmacy.id,ip_pharmacy.medicine_name,ip_pharmacy.medicine_company,ip_pharmacy. medicine_composition,ip_pharmacy.medicine_category_id,ip_pharmacy.medicine_group,ip_pharmacy.unit')
            ->group_by('ip_pharmacy.id')
            ->sort('ip_pharmacy.id', 'desc')
            ->where('`ip_pharmacy`.`medicine_category_id`=`medicine_category`.`id`')
            ->from('ip_pharmacy');
        return $this->datatables->generate('json');
    }
    
    public function get_ip_medicine_name($medicine_category_id)
    {
        $this->db->select('ip_pharmacy.*');
        $this->db->where('ip_pharmacy.medicine_category_id', $medicine_category_id);
        $query = $this->db->get('ip_pharmacy');
        return $query->result_array();
    }
    ...
    public function getIPBatchNumberByBatch($batch){
        ...
        if(!empty($batch)){
             $this->db->select('ip_medicine_batch_details.batch_no,ip_medicine_batch_details.id');
             $this->db->like('batch_no', $batch);
             $this->db->order_by('id','desc');
            $query=$this->db->get('ip_medicine_batch_details');
    ...
    public function getIPSupplierDetails($id)
    {
        $this->db->select('ip_supplier_bill_basic.*,medicine_supplier.supplier,medicine_supplier.supplier_person,medicine_supplier.contact,medicine_supplier.address');
        $this->db->join('medicine_supplier', 'medicine_supplier.id=ip_supplier_bill_basic.supplier_id');
        $this->db->where('ip_supplier_bill_basic.id', $id);
        $query = $this->db->get('ip_supplier_bill_basic');
        return $query->row_array();
    }

    public function getIPAllSupplierDetails($id)
    {
        $query = $this->db->select('ip_medicine_batch_details.*,ip_pharmacy.medicine_name,ip_pharmacy.unit,ip_pharmacy.id as medicine_id,medicine_category.medicine_category,medicine_category.id as medicine_category_id')
            ->join('ip_pharmacy', 'ip_medicine_batch_details.pharmacy_id = ip_pharmacy.id')
            ->join('medicine_category', 'ip_pharmacy.medicine_category_id = medicine_category.id')
            ->where('ip_medicine_batch_details.supplier_bill_basic_id', $id)
            ->get('ip_medicine_batch_details');
        return $query->result_array();
    }
    
    public function deleteIPSupplierBill($id)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $query = $this->db->where("supplier_bill_basic_id", $id)->delete("ip_medicine_batch_details");
        if ($query) {
            $this->db->where("id", $id)->delete("ip_supplier_bill_basic");
        }
        
        $message = DELETE_RECORD_CONSTANT . " On Ip Medicine Batch Details id " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
    ...
    {
        $this->db->select('ip_pharmacy.*,medicine_category.id as medicine_category_id,medicine_category.medicine_category');
        $this->db->join('medicine_category', 'ip_pharmacy.medicine_category_id = medicine_category.id', 'inner');
        $this->db->where('ip_pharmacy.id', $id);
        $this->db->order_by('ip_pharmacy.id', 'desc');
        $query = $this->db->get('ip_pharmacy');
        return $query->row_array();
    }
    
    public function getIPMedicineBatch($pharm_id)
    {
        $this->db->select('ip_medicine_batch_details.*, ip_pharmacy.id as pharmacy_id, ip_pharmacy.medicine_name');
        $this->db->join('ip_pharmacy', 'ip_medicine_batch_details.pharmacy_id = ip_pharmacy.id', 'inner');
        $this->db->where('ip_pharmacy.id', $pharm_id);
        $query = $this->db->get('ip_medicine_batch_details');
        return $query->result();
    }
    ...
    {    
        ...
        if (isset($data['id']) && $data['id'] != 0) {
            $insert_id = $data['id'];
            $this->db->where('id', $data['id'])
                ->update('ip_pharmacy_bill_basic', $data);
                
            $message = UPDATE_RECORD_CONSTANT . " On Ip Pharmacy Bill Basic id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $this->db->insert('ip_pharmacy_bill_basic', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Ip Pharmacy Bill Basic id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
        ...
        if (!empty($delete_array)) {
            ...
            $this->db->where_in('id', $delete_array);
            $this->db->delete('ip_pharmacy_bill_detail');
        }

        if (isset($update_array) && !empty($update_array)) {

            $this->db->update_batch('ip_pharmacy_bill_detail', $update_array, 'id');
        }
        ...
        if (isset($insert_array) && !empty($insert_array)) {
            ...
            $this->db->insert_batch('ip_pharmacy_bill_detail', $insert_array);
        }

        if (isset($payment_array) && !empty($payment_array)) {
            $payment_array['ip_pharmacy_bill_basic_id'] = $insert_id;
            $payment_array['case_reference_id']      = $data['case_reference_id'];
            $this->db->insert("transactions", $payment_array);
        }
    ...
    public function ip_getAllpharmacybillRecord()
    {
        $custom_fields             = $this->customfield_model->get_custom_fields('pharmacy', 1);
        $custom_field_column_array = array();
        ...
        $custom_field_column = (empty($custom_field_column_array)) ? "" : "," . implode(',', $custom_field_column_array);
        $this->datatables
            ->select('ip_pharmacy_bill_basic.*,IFNULL((select sum(amount) as amount_paid from transactions WHERE transactions.ip_pharmacy_bill_basic_id =ip_pharmacy_bill_basic.id and transactions.type="payment" ),0) as paid_amount, IFNULL((select sum(amount) as refund from transactions WHERE transactions.ip_pharmacy_bill_basic_id =ip_pharmacy_bill_basic.id and transactions.type="refund" ),0) as refund_amount, patients.patient_name,patients.id as pid' . $field_variable)
            ->join('patients', 'patients.id = ip_pharmacy_bill_basic.patient_id', 'left')
          
            ->searchable('ip_pharmacy_bill_basic.id,ip_pharmacy_bill_basic.discount,ip_pharmacy_bill_basic.case_reference_id,ip_pharmacy_bill_basic.date,patients.patient_name' . $custom_field_column . ',ip_pharmacy_bill_basic.doctor_name')
            ->orderable('ip_pharmacy_bill_basic.id,ip_pharmacy_bill_basic.case_reference_id,ip_pharmacy_bill_basic.date,patients.patient_name,ip_pharmacy_bill_basic.doctor_name' . $custom_field_column . ',ip_pharmacy_bill_basic.discount,ip_pharmacy_bill_basic.net_amount,paid_amount')
            ->sort('ip_pharmacy_bill_basic.id', 'desc')
            ->from('ip_pharmacy_bill_basic');
           
        return $this->datatables->generate('json');
    }
    
    public function ip_get_medicine_name($medicine_category_id)
    {
        $this->db->select('ip_pharmacy.*');
        $this->db->where('ip_pharmacy.medicine_category_id', $medicine_category_id);
        $query = $this->db->get('ip_pharmacy');
    ...
    public function ip_getBatchNoList($pharmacy_id)
    {
     $sql = "SELECT ip_medicine_batch_details.*, (ip_medicine_batch_details.available_quantity-IFNULL((SELECT SUM(quantity) FROM `ip_pharmacy_bill_detail` WHERE medicine_batch_detail_id=ip_medicine_batch_details.id),0)) as remaining_quantity FROM `ip_medicine_batch_details` WHERE ip_medicine_batch_details.pharmacy_id=".$this->db->escape($pharmacy_id)." HAVING remaining_quantity > 0";
           $query = $this->db->query($sql);
        return $query->result_array();
    }
    
    public function ip_getMedicineBatchByID($medicine_batch_detail_id)
    {
        $sql   = "SELECT ip_medicine_batch_details.*, IFNULL((SELECT SUM(quantity) FROM `ip_pharmacy_bill_detail` WHERE medicine_batch_detail_id=ip_medicine_batch_details.id),0) as used_quantity FROM `ip_medicine_batch_details` WHERE ip_medicine_batch_details.id=" . $this->db->escape($medicine_batch_detail_id);
    ...
    public function ip_getBillDetails($id, $check_print = NULL)
    {
        ...
        $field_variable = implode(',', $field_var_array);       
        $this->db->select('ip_pharmacy_bill_basic.*,IFNULL((select sum(amount) as amount_paid from transactions WHERE transactions.ip_pharmacy_bill_basic_id =ip_pharmacy_bill_basic.id and transactions.type="payment" ),0) as paid_amount, IFNULL((select sum(amount) as refund from transactions WHERE transactions.ip_pharmacy_bill_basic_id =ip_pharmacy_bill_basic.id and transactions.type="refund" ),0) as refund_amount,staff.name,staff.surname,staff.id as staff_id,staff.employee_id,patients.patient_name,patients.id as patientid,patients.id as patient_unique_id,patients.mobileno,patients.age,' . $field_variable);
        $this->db->join('patients', 'ip_pharmacy_bill_basic.patient_id = patients.id');
        $this->db->join('staff', 'ip_pharmacy_bill_basic.generated_by = staff.id');
        $this->db->where('ip_pharmacy_bill_basic.id', $id);
        $query = $this->db->get('ip_pharmacy_bill_basic');
        return $query->row_array();
    }

    public function ip_getAllBillDetails($id)
    {
        $sql = "SELECT ip_pharmacy_bill_detail.*,ip_medicine_batch_details.expiry,ip_medicine_batch_details.pharmacy_id,ip_medicine_batch_details.batch_no,ip_medicine_batch_details.mrp,ip_medicine_batch_details.tax,ip_pharmacy.medicine_name,ip_pharmacy.medicine_group,ip_pharmacy.vat_ac,ip_pharmacy.unit,ip_pharmacy.id as `medicine_id`,ip_pharmacy.medicine_category_id,medicine_category.medicine_category FROM `ip_pharmacy_bill_detail` INNER JOIN ip_medicine_batch_details on ip_medicine_batch_details.id=ip_pharmacy_bill_detail.medicine_batch_detail_id INNER JOIN ip_pharmacy on ip_pharmacy.id= ip_medicine_batch_details.pharmacy_id INNER JOIN medicine_category on medicine_category.id= ip_pharmacy.medicine_category_id WHERE pharmacy_bill_basic_id =" . $this->db->escape($id);
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    public function ip_deletePharmacyBill($id)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $query = $this->db->where("pharmacy_bill_basic_id", $id)->delete("ip_pharmacy_bill_detail");
        if ($query) {
            $this->db->where("id", $id)->delete("ip_pharmacy_bill_basic");
            $this->customfield_model->delete_custom_fieldRecord($id, 'pharmacy');
        }
        
        $message = DELETE_RECORD_CONSTANT . " On Ip Pharmacy Bill Detail id " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
