...
class Itemissue_model extends MY_Model
{
    ...
    {   
        ...
        if (isset($data['id']) && $data['id'] != '') {
            $this->db->where('id', $data['id']);
            $this->db->update('item_consumable', $data);
            $message = UPDATE_RECORD_CONSTANT . " For Item Issue id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $this->db->insert('item_consumable', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Item Issue id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            ...
            if ($this->db->trans_status() === false) {
                ...
                if($record_id){
                    $items = json_decode($data['item_data'],true);
                    foreach($items as $i):
                    $this->db->query('UPDATE item_stock SET quantity = quantity - '.$i['quantity'].' WHERE item_id = "'.$i['item_id'].'"');
                    endforeach;
                }
                //return $return_value;
            }
    ...
    public function getConsumableBillDetails($id){
        $result = array();
        $query = $this->db->query('SELECT item_consumable.*, patients.patient_name, patients.mobileno, ipd_details.case_reference_id, staff.name AS doctor_name
        ...
        WHERE item_consumable.id='.$id);
        $patient_row = $query->row_array();
        $item_obj = json_decode($patient_row['item_data'],true);
        $item_array = array();
        foreach($item_obj as $val):
            if($val['item_category_id']!='' && $val['item_id']!='' && $val['quantity']!=''){
            $iquery = $this->db->query('SELECT item.name AS item_name, item_category.item_category, item_stock.purchase_price 
            FROM item
            JOIN item_category ON item_category.id=item.item_category_id
            JOIN item_stock ON item_stock.item_id=item.id
            WHERE item.id = '.$val['item_id']);
            $item_row = $iquery->row_array();
            $item_array[] = array(
                "item_name" =>$item_row['item_name'],
                "item_category" =>$item_row['item_category'],
                "quantity" =>$val['quantity'],
                "purchase_price" =>$item_row['purchase_price'],
            );
         }
          //return $val->item_id;
        endforeach; 
        $result['result_data'] = $patient_row;
        $result['item_array'] = $item_array;  
        return $result;

    }

    public function deleteConsumableBill($id)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        $this->db->where('id',$id);
            $item_query = $this->db->get('item_consumable');
            $patient_row = $item_query->row_array();
        
        $item_obj = json_decode($patient_row['item_data'],true);
        foreach($item_obj as $val):
            if($val['item_category_id']!='' && $val['item_id']!='' && $val['quantity']!=''){
                $this->db->query('UPDATE item_stock SET quantity = quantity + '.$val['quantity'].' WHERE item_id = "'.$val['item_id'].'"');
            }
        endforeach;
        
        $this->db->where("id", $id)->delete("item_consumable");
           
        
        $message = DELETE_RECORD_CONSTANT . " On Consumable Bill Detail id " . $id;
        $action = "Delete";
        $record_id = $id;
        $this->log($message, $record_id, $action);
        //======================Code End==============================
