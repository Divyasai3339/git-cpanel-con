...
class Customlib
{
    ...
    public function getSurgerySlotByDoctorShift($doctor_id, $shift)
    {
        $this->CI->load->model("surgeryappointment_model");
        $shift             = $this->CI->surgeryappointment_model->getShiftById($shift);
        
        $starttime         = $shift["start_time"];
        $endtime           = $shift["end_time"];
        //$shift_details     = $this->CI->surgeryappointment_model->getShiftDetails($doctor_id);
        $duration          = 15;
        $array_of_time     = array();
        $start_time        = strtotime($starttime);
        $end_time          = strtotime($endtime);
   ...
   public function dateFormat($strtotime = null)
    ...
    function getMedicine_expire_month($date){
        ...
        function getProduct_expire_month($date){
        return date('M/Y',strtotime($date));
  }
