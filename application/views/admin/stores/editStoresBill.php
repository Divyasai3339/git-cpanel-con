...
<?php } ?>
                ...
                <script type="text/javascript">
                    ...
                    function getneweditExpire(id) {
                        var med_id = $("#product_edit_name" + id).val();
                        var batch_no = $("#batch_edit_no" + id).val();
                        $('#edit_expire_date' + id).val('');
                        $.ajax({
                            type: "POST",
                ...
                <div class="col-md-12" style="clear: both;">
                    <div class="table-responsive">
                        <table class="table tableover table-striped table-bordered table-hover tablefull12 tblProducts" id="edittableID">
                            <tr >
                                <th><?php echo $this->lang->line('product') . " " . $this->lang->line('category'); ?><small class="req" style="color:red;"> *</small></th>
                                <th><?php echo $this->lang->line('product') . " " . $this->lang->line('name'); ?><small class="req" style="color:red;"> *</small></th>
                            ...
                            foreach ($detail as $key => $value) {
                                ...
                                <script type="text/javascript">
                                    ...
                                    geteditbatchnolist('<?php echo $value['product_id'] ?>', '<?php echo $i ?>', '<?php echo $value['batch_no'] ?>');
                                    geteditQuantity('<?php echo $i ?>','<?php echo $value['product_id'] ?>','<?php echo $value['batch_no'] ?>');
                                </script>
                                <tr id="row<?php echo $i ?>">
                                    <td width="16%"> 
                                        <input name="id" type="hidden" class="form-control" value="<?php echo $value['id']; ?>" />
                                        <select class="form-control product_category" name='product_category_id[]'  onchange="getproduct_edit_name(this.value, '<?php echo $i ?>', '<?php echo $value['product_name'] ?>')">
                                    ...
                                    <td width="24%">
                                        <select class="form-control select2 product_name" style="width: 100%" id="product_edit_name<?php echo $i ?>" name='product_name[]' onchange="geteditbatchnolist(this.value, '<?php echo $i ?>')">
...
<script type="text/javascript">
    ...
    function editMore() {
        ...
        var div = "<td><select class='form-control product_category' name='product_category_id[]' onchange='getproduct_edit_name(this.value," + id + ")'><option value='<?php echo set_value('product_category_id'); ?>'><?php echo $this->lang->line('select') ?></option><?php foreach ($productCategory as $dkey => $dvalue) { ?><option value='<?php echo $dvalue["id"]; ?>'><?php echo $dvalue["product_category"] ?></option><?php } ?></select></td><td><select class='form-control select2 product_name' name='product_name[]' onchange='geteditbatchnolist(this.value," + id + ")' id='product_edit_name" + id + "' ><option value='<?php echo set_value('product_name'); ?>'><?php echo $this->lang->line('select') ?></option></select></td><td><select name='batch_no[]' onchange='getneweditExpire(" + id + ")' id='batch_edit_no" + id + "'  class='form-control batch_no'><option></option></select></td><td><input type='text' id='edit_expire_date" + id + "' readonly name='expire_date[]' class='form-control'></td><td><div class='input-group'><input type='text' name='quantity[]' id='quantity_edit" + id + "' class='form-control text-right qty'><span class='input-group-addon text-danger'  id='totaleditqty" + id + "'>&nbsp;&nbsp;</span></div><input type='hidden' name='available_quantity[]' id='available_edit_quantity" + id + "'><input type=hidden class=form-control value='0' name='bill_detail_id[]'  /><input type='hidden' name='product_batch_id[]' id='product_batch_id" + id + "'></td><td> <input type='text' name='sale_price[]' id='sale_price_edit" + id + "'  class='form-control text-right price'></td><td><input type='text' name='amount[]' id='amount_edit" + id + "'  class='form-control text-right subtot'></td>";
    ...
    $(document).ready(function (e) {
        $("#editbill").on('submit', (function (e) {
            ...
            e.preventDefault();
            $.ajax({
                url: '<?php echo base_url(); ?>admin/stores/updateBill',
                type: "POST",
