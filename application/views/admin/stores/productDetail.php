...
<div class="nav-tabs-custom border0" id="tabs">
    <ul class="nav nav-tabs navlistscroll">
        ...
        <?php  } if ($this->rbac->hasPrivilege('product_bad_stock', 'can_view')) { ?>
            <li class="">
                            ...
                            <tbody>
                                ...
                                foreach ($result as $detail) {
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $this->customlib->getProduct_expire_month($detail->expiry); ?></td> 
                                        ...
                                        <td class="text-right"><?php if ($this->rbac->hasPrivilege('product', 'can_delete')) { ?><a href="#" class="btn btn-default btn-xs" data-toggle="tootip" title="<?php echo $this->lang->line('delete'); ?>" onclick="delete_batch('<?php echo $detail->id ?>', '<?php echo $detail->stores_id ?>')"><i class="fa fa-trash"></i></a><?php } ?></td>
                                    </tr>
                                    <?php
                                    $count++;
                                }
                        ...
                        <tbody>
                            ...
                            foreach ($badstockresult as $stockdetail) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $this->customlib->getProduct_expire_month($stockdetail->expiry_date); ?></td> 
                                    <td class="text-right"><?php echo $stockdetail->quantity ?></td>
                                    <td class="text-right"><?php if ($this->rbac->hasPrivilege('product_bad_stock', 'can_delete')) { ?> <a href="#" class="btn btn-default btn-xs" data-toggle="tootip" title="<?php echo $this->lang->line('delete'); ?>" onclick="delete_badstock('<?php echo $stockdetail->id ?>', '<?php echo $stockdetail->stores_id ?>', '<?php echo $stockdetail->product_batch_details_id ?>')"><i class="fa fa-trash"></i></a><?php } ?></td>
                                </tr>
                                <?php
                            } 
                            ?>
    ...
    <script type="text/javascript">
        ...
        function delete_batch(id, stores_id) {          
            if (confirm('<?php echo $this->lang->line('are_you_sure'); ?>')) {
                $.ajax({
                    url: '<?php echo base_url(); ?>admin/stores/delete_product_batch/' + id,
                    type: "POST",
                    data: {opdid: ''},
                    dataType: 'json',
                    success: function (data) {
                        if (data.status == 'success') {
                            viewDetail(stores_id, id);
                        } else {
        ...
        function delete_badstock(id, stores_id, product_batch_details_id) {
            if (confirm('<?php echo $this->lang->line('are_you_sure'); ?>')) {
                $.ajax({
                    url: '<?php echo base_url(); ?>admin/stores/deleteBadStock/' + id + '/' + product_batch_details_id,
                    type: "POST",
                    data: {opdid: ''},
                    dataType: 'json',
                    success: function (data) {
