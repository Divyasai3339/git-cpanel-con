            ...
            <div class="col-md-10">              
                <div class="box box-primary" id="tachelist">
                    <div class="box-header ptbnull">
                        <h3 class="box-title titlefix"><?php echo $this->lang->line('product_dosage_list'); ?></h3>
                        <div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('product_dosage', 'can_add')) { ?>
                                <a onclick="addModal()" class="btn btn-primary btn-sm product"><i class="fa fa-plus"></i>  <?php echo $this->lang->line('add_product_dosage'); ?></a> 
                            <?php } ?>    
                        </div>
                    </div> 
                    <div class="box-body">
                        ...
                        <div class="table-responsive mailbox-messages">
                            <div class="download_label"><?php echo $this->lang->line('product_dosage_list'); ?></div>
                            <table class="table table-striped table-bordered table-hover example" >
                                ...
                                <tbody>
                                    <?php
                                    $count = 1;
                                    if (!empty($productDosage)) {
                                        foreach ($productDosage as $dosage) {
                                           ...
                                           $subcount = 1;
                                            ...
                                            ?>
                                            <tr>
                                                <td><?php if($subcount==1){ echo $value['product_category']; } ?></td>
                                                <td><?php echo $value['dosage']." ".$value['unit']; ?></td>
                                                
                                               
                                                <td class="text-right">
                                                    <?php  if ($this->rbac->hasPrivilege('product_dosage', 'can_edit')) { ?>
                                                    ...
                                                    if ($this->rbac->hasPrivilege('product_dosage', 'can_delete')) {
                                                        ?>
                                                        <a  class="btn btn-default btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="delete_product_dosage('<?php echo $value['id'] ?>')">
    ...
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                ...
                <h4 class="modal-title"><?php echo $this->lang->line('add_product_dosage'); ?></h4> 
            </div>
            <form id="formadd" action="<?php echo site_url('admin/productdosage/add') ?>"  id="employeeform" name="employeeform" method="post" accept-charset="utf-8">
                
                <div class="scroll-area">
                    <div class="modal-body pt0 pb0">
                        <div class="ptt10">
                            <div class="form-group">
                                <label for="exampleInputEmail1"><?php echo $this->lang->line('product_category'); ?></label><small class="req"> *</small>
                                <select name="product_category" placeholder=""  onchange="getProductName(this.value)" type="text" class="form-control">
                                    <option value=""><?php echo $this->lang->line('select'); ?></option>
                                    <?php foreach ($productCategory as $key => $catvalue) {
                                        ?>
                                        <option value="<?php echo $catvalue["id"] ?>"><?php echo $catvalue["product_category"] ?></option>
                                    <?php } ?>
                                </select>
...
<div class="modal fade" id="editmyModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title"><?php echo $this->lang->line('edit_product_dosage'); ?></h4> 
            </div>



            <form id="editformadd" action="<?php echo site_url('admin/productdosage/add') ?>" name="employeeform" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
                <div class="modal-body pt0 pb0">
                    <div class="ptt10">

                        <div class="form-group">
                            <label for="exampleInputEmail1"><?php echo $this->lang->line('category_name'); ?></label><small class="req"> *</small>
                            <select name="product_category" id="product_category" placeholder=""  onchange="editProductName(this.value)" type="text" class="form-control">
                                <option value=""><?php echo $this->lang->line('select') ?></option>
                                <?php foreach ($productCategory as $key => $catvalue) {
                                    ?>
                                    <option value="<?php echo $catvalue["id"] ?>"><?php echo $catvalue["product_category"] ?></option>
...
<script> 
    ...
    function get(id) {
        ...
        $.ajax({
            ...
            url: '<?php echo base_url(); ?>admin/productdosage/get_data/' + id,

            success: function (result) {
                $('#dosageid').val(result.id);
                $('#dosage').val(result.dosage);
                $('#unit').val(result.charge_units_id);
                $('#product_category').val(result.product_category_id);
            }
...
<script>
    ...
    function delete_product_dosage(id){
        delete_recordByIdReload('admin/productdosage/delete/'+id, '<?php echo $this->lang->line('delete_confirm'); ?>');
    }
</script>