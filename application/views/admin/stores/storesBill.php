            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title titlefix"> <?php echo $this->lang->line('stores_bill'); ?></h3>
                        <div class="box-tools pull-right">
                            <a href="<?php echo base_url(); ?>admin/optometry/acceptance" class="btn btn-primary btn-sm"><i class="fa fa-reorder"></i> <?php echo $this->lang->line('acceptance'); ?></a>
                            <?php if ($this->rbac->hasPrivilege('stores_bill', 'can_add')) {?>  
                            ...
                            <?php }?>
                            <?php if ($this->rbac->hasPrivilege('product', 'can_view')) {?>
                                <a href="<?php echo base_url(); ?>admin/stores/search" class="btn btn-primary btn-sm"><i class="fa fa-reorder"></i> <?php echo $this->lang->line('products'); ?></a>
                            <?php }?>
                        </div>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <div class="download_label"><?php echo $this->lang->line('stores_bill'); ?></div>
                     <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover ajaxlist" cellspacing="0" width="100%" data-export-title="<?php echo $this->lang->line('stores_bill'); ?>"> <thead>
                                <tr>
...
<script id="medcat-template" type="text/template">
   <?php
foreach ($productCategory as $dkey => $med_cat_value) {
    ?>
    <option value='<?php echo $med_cat_value["id"]; ?>'>
        <?php echo $med_cat_value["product_category"] ?>
    </option>
    <?php
     }
   ?>
...
<script type="text/javascript">
         var datetime_format = '<?php echo $result = strtr($this->customlib->getHospitalDateFormat(true, true), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY', 'H' => 'hh', 'i' => 'mm']) ?>';
                ...
                $(document).on('input paste keyup','.tax_percent,.discount_percent,.qty,.product_category,.product_name,.batch_no,.price,.mamount', function(e){ 
           ...
           function update_amount(__this){
            ...
            $tblrows.each(function (index) {
                    ...
                    var tax_percentage = $tblrow.find(".product_tax").val();
             ...
             var $mtblrows = __this.find(".mtblProducts tbody tr");
            $mtblrows.each(function (index) {
                    var $mtblrows = $(this);  
                    var tax_percentage = $mtblrows.find(".product_tax").val();
                    var mamount = $mtblrows.find(".mamount").val();
                    var subTotal = parseFloat(mamount);

                   
                    if (!isNaN(subTotal)) {
                            $mtblrows.find('.subtot').val(subTotal.toFixed(2));
                            var tax_amount=(subTotal * tax_percentage / 100 );
                            total_tax_amount += isNaN(tax_amount) ? 0 : tax_amount;                       
                            var stval = parseFloat(subTotal.toFixed(2));
                            grandTotal += isNaN(stval) ? 0 : stval;
...
function getPrescriptionData(){
  ...
  $.ajax({
        url: '<?php echo base_url(); ?>admin/stores/getPrescriptionById',
        type: "POST",
...
errorMsg(res.msg);
      ...
      $("#bill .patient_list_ajax").trigger({
          ...
          params: {
           ...
           total_rows=res.total_rows;
            ...
            if(res.total_rows > 0){
              var productTable=$("#billModal .modal-body").find('table.tblProducts');
                //=============
              productTable.find("tbody tr").each(function() {
                var product_category_obj = $(this).find("td select.product_category");
                var product_obj = $(this).find("td select.product_name");
                var batch_obj = $(this).find("td select.batch_no");
                var post_product_category_id = $(this).find("td input.post_product_category_id").val();
                var post_product_id = $(this).find("td input.post_product_id").val();
                var post_product_batch_detail_id = $(this).find("td input.post_product_batch_detail_id").val();
                var post_product_sale_price = $(this).find("td input.sale_price").val();
                var post_product_quantity = $(this).find("td input.quantity").val();
                var product_array = {};
                product_array['quantity'] = post_product_quantity;
                product_array['sale_price'] = post_product_sale_price;
                $('.select3').select2();
                $('.filestyle','#billModal').dropify();
                getProduct(product_category_obj,post_product_category_id,post_product_id);
                getBatchNo(product_obj,post_product_id,post_product_batch_detail_id);
              });
            }
      }     
  ...
  $(document).on('click','.generatebill',function(){
      ...
      $.ajax({
          url: '<?php echo base_url(); ?>admin/stores/createBill',
          type: "POST",
          dataType: 'json',
           beforeSend: function() {
              $this.button('loading');
   ...
   $(document).on('click','.add_payment',function(){  
    ...
    function getPayments(record_id){
         var payment_modal=$('#addPaymentModal');
        $.ajax({
            url: '<?php echo base_url() ?>admin/stores/getStoresTransaction',
            type: "POST",
            data: {'id': record_id},
            dataType:"JSON",
            beforeSend: function(){
      ...
      $this.button('loading');
      $.ajax({
          url: '<?php echo base_url(); ?>admin/stores/printTransaction',
          type: "POST",
          data:{'id':record_id},
          dataType: 'json',
           beforeSend: function() {
    ...
    function getProduct(med_cat_obj,val,product_id){
      var product_colomn=med_cat_obj.closest('tr').find('.product_name');
        product_colomn.html("");    
        $.ajax({
            url: '<?php echo base_url(); ?>admin/stores/get_product_name',
            type: "POST",
            data: {product_category_id: val},
            dataType: 'json',
             async: false,
              beforeSend: function() {
              product_colomn.html("<option value=''><?php echo $this->lang->line('loading') ?></option>");
            ...
            success: function (res) {
                ...
                $.each(res, function (i, obj)
                {
                    var sel = "";
                            ...
                            div_data += "<option value=" + obj.id + " " + sel + ">" + obj.product_name + "</option>";

                });
           
                product_colomn.html(div_data);
                product_colomn.select2("val", product_id);
 ...
 $(document).on('select2:select','.product_name',function(){
     getBatchNo($(this),$(this).val(),0);
    });

 function getBatchNo(med_obj,stores_id,batch_id){
 ...
 $(document).on('select2:select','.batch_no',function(){
    var product_details = {};
      getProductDetail($(this),$(this).val(),product_details);
    });

function getProductDetail(batch_obj,product_batch_detail_id,product_details){
      var current_row =batch_obj.closest('tr');
        $.ajax({
           type: "POST",
            url: base_url + "admin/stores/getExpiryDate",
            data: {'product_batch_detail_id': product_batch_detail_id},
            dataType: 'JSON',
            asyn:true,
            ...
            success: function (res) { 
               ...
               current_row.find('.product_tax').val(res.tax);
               
                   if($.isEmptyObject(product_details)){
                   ...
                   current_row.find('.price').val(res.sale_rate);
                    update_amount(batch_obj.closest('div.modal'));
                       ...
                       var total_price=product_details.sale_price*product_details.quantity
                       current_row.find('.available_quantity').val(parseInt(quantity_remaining)+parseInt(product_details.quantity));
                       current_row.find('.qty').val(product_details.quantity);
                       current_row.find('.price').val(product_details.sale_price);
...
var table = document.getElementById("tableID");
 ...
 $("form#bill button[type=submit]").click(function() {
        ...
        $("#bill").on('submit', (function (e) {
            ...
            var form = $(this);            
            $.ajax({
                url: baseurl +'admin/stores/addBill',
                type: "POST",
    ...
    function viewDetail(id) {
        ...
        $.ajax({
            url: '<?php echo base_url() ?>admin/stores/getBillDetails/',
            type: "GET",
            data: {'id': id},
            dataType:"JSON",
            beforeSend: function(){
    ...
    $(document).on('click','.edit_bill',function(){
          ...
          var prescription_no=$(this).data('prescriptionId');
              $.ajax({
            url: '<?php echo base_url() ?>admin/stores/editBill/',
            type: "POST",
            data: {'id': record_id},
            dataType:"JSON",
            beforeSend: function(){
           ...
           var update_date = new Date(data.date);
            ...
            product_array['sale_price'] = post_product_sale_price;
             ...
             getProduct(product_category_obj,post_product_category_id,post_product_id);
             getBatchNo(product_obj,post_product_id,post_product_batch_detail_id);
             getProductDetail(batch_obj,post_product_batch_detail_id,product_array);
...
$(".modalbtnpatient").click(function(){
 ...
 $(document).on('click','.delete-record',function(){
   ...
   if (confirm(message)) {
       $.ajax({
        url: base_url+'admin/stores/deleteStoresBill',
        type: "POST",
        data:{'id':delete_id},
        dataType: 'json',
         beforeSend: function() {
...
( function ( $ ) {
    'use strict';
    $(document).ready(function () {
        initDatatable('ajaxlist','admin/stores/getstoresbillDatatable',[],[],100,
            [
...
<script type="text/javascript">
        ...
        $(document).on('submit','#add_partial_payment', function(e){
            ...
            var stores_bill_basic_id=$("input[name='stores_bill_basic_id']",'#add_partial_payment').val();
               ...
               data: new FormData(this),
                ...
                success: function (data) {
                    if (data.status == "fail") {
                        ...
                        successMsg(data.message);
                         getPayments(stores_bill_basic_id);
                        }
                     btn.button('reset');
                },
                error: function () {
          ...
          $(document).on('click','.delete_trans', function(e){
            ...
            $.ajax({
                ...
                type: "POST",
                data: {'id':record_id,'stores_bill_basic_id':stores_bill_basic_id},
                dataType: 'JSON',               
                success: function (data) {
                    successMsg(data.message);
                    getPayments(stores_bill_basic_id);
...
<script>
    ...
    function dateChanged(ev) {            
        ...
        //added by chitranjan
var mtotal_rows=1;
$(document).on('click','.madd-record',function(){
var table = document.getElementById("mTableID");
        var id = mtotal_rows+1;
        var div = "<td><input type='hidden' name='mtotal_rows[]' value='" + id + "'><input type='text' class='form-control' name='mproduct_name[]' id=''></td><td ><input type='text' name='mdescription[]'  id='mdescription' class='form-control'/></td> <td class='text-right w-100px'><input type='text' name='mamount[]' id='mamount' placeholder='' class='form-control mamount'></td>";

        var row =  "<tr id='mrow" + id + "'>" + div + "<td><button type='button' data-mrow-id='"+id+"' class='closebtn mdelete_row'><i class='fa fa-remove'></i></button></td></tr>";
        $('#mTableID').append(row);
        mtotal_rows++;
       
});
 
  $(document).on('click','.mdelete_row',function(e){
        var modal_=$(e.target).closest('div.modal');
        var del_row_id=$(this).data('mrowId');
        $("#mrow" + del_row_id).remove();
        update_amount(modal_);
  });
