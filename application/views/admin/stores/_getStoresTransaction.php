...
$currency_symbol = $this->customlib->getHospitalCurrencyFormat();
 if($view_delete){ 
    ...
    $print_receipt="print_receipt";
     ...
     $print_receipt="print_storesBillReceipt";
           ...
           <table class="table table-hover table-sm">
            <tbody>
                <tr>
                    ...
                    <td><?php echo $this->customlib->getSessionPrefixByType('stores_billing').$stores_bill_detail['id']; ?></td>
                </tr>
                <tr>
                    <td><label><?php echo $this->lang->line('case_id');?></label></td>
                    <td><?php echo $stores_bill_detail['case_reference_id']; ?></td>
                </tr>
                <tr>     
                    <td><label><?php echo $this->lang->line('patient');?></label></td>
                    <td><?php echo composePatientName($stores_bill_detail['patient_name'],$stores_bill_detail['patient_id']); ?></td>
                </tr>
                <tr>           
                    <td><label><?php echo $this->lang->line('generated_by');?></label></td>
                    <td><?php echo composeStaffNameByString($stores_bill_detail['name'],$stores_bill_detail['surname'],$stores_bill_detail['employee_id']); ?></td>  
                </tr>
                <tr>    
                    <td><label><?php echo $this->lang->line('date');?></label></td>
                    <td><?php echo $this->customlib->YYYYMMDDHisTodateFormat($stores_bill_detail['date']); ?></td>          
                </tr>
                <tr>
                    <td><label><?php echo $this->lang->line('mobileno');?></label>Mobile</td>
                    <td><?php echo $stores_bill_detail['mobileno']; ?></td>
                </tr>
            </tbody>
            </table>  
         </div>   
        ...
        <table class="table table-hover table-sm">
            <tbody> 
                  <tr>
                    ...
                    <td class="text-right"><?php echo $stores_bill_detail['total']; ?></td>
                </tr>  
                <tr>
                    <td class="text-right"><label><?php echo $this->lang->line('discount_percentage'). " (" . $currency_symbol . ")";?></label></td>
                    <td class="text-right"><?php echo "(".$stores_bill_detail['discount_percentage'] ."%) ".$stores_bill_detail['discount']; ?></td>
                </tr>
                <tr>
                    <td class="text-right"><label><?php echo $this->lang->line('tax'). " (" . $currency_symbol . ")";?></label></td>
                    <td class="text-right"><?php echo $stores_bill_detail['tax']; ?></td>
                </tr>
                <tr>
                    <td class="text-right"><label><?php echo $this->lang->line('net_amount'). " (" . $currency_symbol . ")";?></label></td>
                    <td class="text-right"><?php echo $stores_bill_detail['net_amount']; ?></td>
                </tr>    
                
            <tr>
                <td class="text-right"><label><?php echo $this->lang->line('paid_amount'). " (" . $currency_symbol . ")";?></label></td>
                <td class="text-right"><?php echo $stores_bill_detail['paid_amount']; ?></td>
            </tr>

             <tr>
                <td class="text-right"><label><?php echo $this->lang->line('refund_amount'). " (" . $currency_symbol . ")";?></label></td>
                <td class="text-right"><?php echo $stores_bill_detail['refund_amount']; ?></td>
            </tr>
            <tr>
                <td class="text-right"><label><?php echo $this->lang->line('due_amount'). " (" . $currency_symbol . ")";?></label></td>
                <td class="text-right"><?php echo amountFormat(($stores_bill_detail['net_amount'] + $stores_bill_detail['refund_amount'] )-$stores_bill_detail['paid_amount']); ?></td>
                    ...
                    <div class="col-lg-5 col-md-5 col-sm-5">
                        <?php  if ($this->rbac->hasPrivilege('stores_billing_payment', 'can_add') || $this->rbac->hasPrivilege('stores_partial_payment', 'can_add')) { ?>
                        <form id="<?php echo $form_id; ?>" action="<?php echo site_url('admin/stores/partialbill')?>" accept-charset="utf-8" method="post"  >
                            <input type="hidden" name="stores_bill_basic_id" value="<?php echo $stores_bill_basic_id;?>">
                            <div class="row">
                                   <div class="col-md-6">
                                    <div class="form-group">
                                        <label><?php echo $this->lang->line('date'); ?></label><small class="req"> *</small> 
                                        <input type="text" name="payment_date" id="date" class="form-control datetime">
                                        <input type="hidden" name="patient_id" value="<?= $stores_bill_detail['patient_id']; ?>">
                                        <input type="hidden" name="case_reference_id" value="<?= $stores_bill_detail['case_reference_id'] ; ?>">
                                         <input type="hidden" name="refund_amount" value="<?= $stores_bill_detail['refund_amount'] ; ?>">
...
<div class="table-responsive">
 ...
 <table class="table table-hover">
	...
	<tbody>
		<?php 
foreach ($stores_transaction as $transaction_key => $transaction) {
	?>
<tr>
    ...
    <a href='javascript:void(0)' data-loading-text='<i class="fa fa-circle-o-notch fa-spin"></i>' data-record-id="<?php echo $transaction->id;?>" class='btn btn-default btn-xs <?php echo $print_receipt; ?>'  data-toggle='tooltip' title='<?php echo $this->lang->line('print'); ?>'><i class='fa fa-print'></i></a>
        <?php if($view_delete){
            if($this->rbac->hasPrivilege('stores_partial_payment', 'can_delete')) { ?>
